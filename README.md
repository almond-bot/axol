...
1. In the web control panel (`axol serve`), the **Quest USB** tile shows the headset state. Click **Connect** — this runs `adb reverse tcp:8000 tcp:8000` and, the first time, pops an *Allow USB debugging?* prompt on the headset; accept it (check "Always allow"). The tile turns green once the tunnel is up.
