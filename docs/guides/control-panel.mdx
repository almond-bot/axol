...
| **Collect data** | Axol or Mantis + cameras | Record Axol joint or Cartesian actions, or (with Mantis selected on its tile) Mantis Cartesian TCP actions. Production Mantis start is blocked without a ready source and approved factory or measured transforms. The **repo id** picker can resume host datasets; feeds and [episode controls](#episode-controls) are mirrored in the panel. See the [Mantis collection flow](/operations/data-collection#mantis-collection-end-to-end). |
