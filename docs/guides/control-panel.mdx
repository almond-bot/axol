...
The panel inventories the host's configured CAN interfaces when it comes online and while the link is idle. If only Axol or only Mantis is present, it connects that hardware automatically; if both are present, the profile required by the selected operation wins. A present-but-down interface still qualifies because Connect brings it up. On a previously configured host, the exact USB serial saved in that profile's udev rules also qualifies before `gs_usb` has loaded or created the named interfaces; connecting then restores the driver and names. The panel never guesses whether an arbitrary `can0` is Axol or Mantis — a new, unassigned hub still needs one deliberate **Connect** or `axol can.setup` so its role can be established safely. A hub that enumerates shortly after page load is picked up by the inventory poll. A failed automatic connection is retried twice with backoff; after that, the tile's **Connect** button is the manual fallback.
