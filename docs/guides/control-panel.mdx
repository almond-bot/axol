...
The **Start** button stays disabled until everything an operation needs is ready, and a "Before you can start" list spells out what's missing — e.g. *Connect Axol*, *Fix motor fault: R elbow — over temperature*, *Assign at least one camera*, *Identify left + right Lighthouse trackers*, *Fix Ultimate private shared-map AP setup*, or *Add an approved factory or measured tracker-to-TCP transform*. Fix tracker setup in **Mantis settings → Tracking** ([Mantis Tracking](/mantis/tracking)); transforms are a [file on the host](/cli/tracker#transform-file). The panel refreshes the same readiness the operation start uses.
