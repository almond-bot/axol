...
| **Robot** | Left/right arm stiffness sliders, CAN channels, gripper torque limit and max speed (both grippers), gravity-comp damping and rate, the **contact-stop** thresholds (the always-on reset guard plus the opt-in teleop and policy watchdogs — each a switch that arms it at a suggested value), and the **Jelly** toggle (drives [Jelly](/guides/vr-interface#jelly) from the headset thumbsticks in teleop and data collection). |
...
| **Recording** | Dataset fps, video codec, dataset root, push-to-hub, Rerun viewer address. Dataset/policy fps is independent of the fixed 30 fps headset stream. Cameras may capture faster and are decimated for recording; setting it above the default 60 raises physical capture to match (the ZED X only supports higher rates at SVGA). |
