...
The **Axol** and **Mantis** tiles also carry the one system-wide choice of which hardware every operation runs on: the selected device's tile shows a **selected** marker and the other offers **Select**. There is no per-operation Mantis toggle: teleop and data collection follow the selection, the idle link auto-connects to the selected device, and while Mantis is selected only **Teleoperation** and **Collect data** are listed — the Axol-only operations (gravity comp, waypoints, replay, run policy, DAgger) return when Axol is selected again. The selection is stored with the [shared settings](#settings) on the host, so every operator device sees the same choice; it is locked while an operation is running.

### Axol Host

The **Axol Host** is the machine running `axol serve` — the computer wired to the robot, with the ZED cameras attached. Press **Connect** on the tile to open the host dialog and enter its address (just the IP — port `8001` is assumed). Click the field for a dropdown of the last five hosts this browser connected to successfully, newest first; pick one or keep typing, and press **Escape** to close the list without closing the dialog; if the host can't be reached the attempt gives up after a couple of seconds with a *can't reach the host* message instead of leaving the tile stuck on *Connecting…*. When connected, the tile shows the host's name, its installed release version (e.g. `axol v0.1.2`), and a green dot. Teleop and gravity comp need only this connection.

#### Axol version

Axol comes in two versions with the same arms: **Axol Classic** (fixed mount) and **Axol Mobile** (on the Jelly base, with the head and wrist cameras). There is nothing to select — a robot is the mobile Axol whenever [Jelly](/guides/vr-interface#jelly) is enabled, that is, its wheel or lift adapter is attached and not switched off in the Robot tab. Every operation then uses the mobile robot model: it sets the body the IK solver keeps the arms clear of (the lift column, its top plate, and the head camera) and the joint limits (the mobile elbows reach 24° further past straight). Gravity compensation, recorded end-effector poses and the tuned per-joint masses are identical on both. The **Rest pose** viewer shows whichever version the host is running. To force one, set `kinematics.robot_model` to `classic` or `mobile` under **Advanced → Kinematics** (or pass `--kinematics.robot_model`).

**Whole-body IK.** By default only the arms are IK joints (arm IK). The **Whole-body IK (sim only)** switch in the **Kinematics** tab (or `--kinematics.whole_body true`) adds the Jelly base (forward, sideways, rotation) and the lift: while both hands stay within comfortable reach the body holds still, and when a target goes beyond it the base drives toward it or the lift lowers for low targets. It works in relative (Quest) teleop with **sim** on; the sim viewer shows the base and lift moving. Driving the real wheels and lift from the IK, absolute (Mantis) mode, and data collection are not supported yet — those runs refuse to start with the switch on.
...
When a newer [release](https://github.com/almond-bot/axol/releases) than the installed one exists, a banner appears above the tiles showing the version jump (e.g. `v0.1.2 → v0.1.3`) with a link to the release notes. Press **Update** to apply it: the host reinstalls at the new release, re-provisions its system dependencies, and restarts once idle (or reboots the host, when a step such as a new camera driver only takes effect at boot) — the button is disabled while an operation is running, since applying an update restarts the server. Nothing updates without that button press.
...
The panel inventories the host's CAN interfaces when it comes online and while the link is idle. A new dual-channel hub may initially appear only as anonymous `can0`/`can1` interfaces; before connecting, the root server runs the same driver/setup identification path as `axol can.setup`, safely probes the attached shoulder motor and Mantis trigger, saves the proven role, and assigns the persistent interface names. The same pass covers Jelly: a new single-channel adapter is probed for the Damiao wheel motors and the jelly_legs lift controller and pinned as the wheel bus (`can_alm_axol_b`), the chest/lift bus (`can_alm_axol_c`), or the shared wheel + lift bus — see [`can.setup`](/cli/can-setup) for the topologies. The probe is bounded and shared across browser tabs, so an unpowered or ambiguous adapter is never guessed or repeatedly reset. In that case, power the attached hardware and press **Retry CAN identification**; use interactive `axol can.setup` in a terminal only when the role still cannot be proven automatically. An already pinned Jelly adapter whose base is powered off keeps its assignment and does not hold up connecting the arms. Unrelated arbitrary SocketCAN interfaces are never classified as Axol, Mantis, or Jelly automatically.
...
**A cut motor supply recovers on its own.** If motor power drops mid-operation — operators use the PSU as an e-stop — the run stops with a `motor power lost` status instead of appearing to keep running, and the affected motors read **unknown** rather than a frozen pre-cut value (unknown motors are not counted as faults; unreachable ones still are). If the operation could not confirm the motors went torque-free, the server probes them itself as the run ends and releases the robot when every motor reads disabled or unpowered — the common case needs nothing from you. Only when a motor still answers under torque (or none could be read) does the panel keep the robot reserved and show a **Re-check motors** button: cut motor power and press it to re-probe the motors and release the reservation. Host restart and shutdown are never blocked by this reservation.
...
**Jelly battery.** While Jelly's lift link is connected, the **Jelly** tile's header shows the battery charge next to the pack voltage: green, amber at 25% and below, red at 10% and below, blue while charging. The lift controller measures the 24 V rail once a second, and the panel converts that resting voltage to a charge level using LiTime's resting-voltage chart for Jelly's two 24 V 50 Ah LiFePO4 packs. Readings taken while the lift or wheels are moving sag, so they never override a resting reading. A reading that only exists under load is shown as `~40%`. While a task owns the bus, the tile keeps the last reading, dimmed, and hovering it shows how old it is. LiFePO4 voltage barely changes above about 25% charge, so treat the number as a band rather than a fuel gauge. The packs' Bluetooth app (LiTime) shows the BMS's own figure if you need an exact one. From Python, [`Jelly.battery`](/api/robot#jelly) and `read_battery()` return the same estimate.

For **Mantis**, press **Select** on the Mantis tile — see [Mantis Hardware](/mantis/hardware) for the rigs and hub. **Teleop needs only CAN and never tracks**: it drives the grippers straight from the rig triggers. **Collection** uses the pose source set up under **Mantis settings → Tracking** ([Mantis Tracking](/mantis/tracking) covers the Quest / Lighthouse / Ultimate setup flows and the start gesture); the panel launches and owns the selected tracker bridge and its two trigger readers, and a missing binding, incomplete channel mapping, or backend failure is a visible blocker rather than an apparently running session with no input. Teleop enables the physical grippers immediately; collection keeps them motor-disabled between takes. Mantis is not available for Collect DAgger or Run Policy.
...
| **Robot** | Left/right arm stiffness sliders, CAN channels, per-gripper torque limit and max speed, gravity-comp damping and rate, the **contact-stop** thresholds (the always-on reset guard plus the opt-in teleop and policy watchdogs — each a switch that arms it at a suggested value), and the three hardware switches — **Axol arms**, **Jelly wheels**, **Jelly lift**. What a session drives is inferred from the CAN devices attached (arm hub, [Jelly](/guides/vr-interface#jelly) wheel bus, lift bus — see [Axol and Mantis](#axol-and-mantis)); these switches opt attached hardware *out*. Jelly enabled also selects the [mobile robot model](#axol-version). Arms off drives just Jelly from the headset thumbsticks with no Axol connection or motor check needed. |
...
| Tab | What's in it |
|---|---|
| **Tracking** | The Quest/Lighthouse/Ultimate pose source and its step-by-step setup flow (each step gated on host-reported readiness), ending once the sides are identified. See [Mantis Tracking](/mantis/tracking). |
| **Cameras** | The Left Mantis / Right Mantis camera layout, stored separately from the Axol layout. |
| **CAN** | The logical left/right Mantis CAN interfaces (`can_mantis_l` / `can_mantis_r` by default); swap the two values to swap sides without moving cables. |

**General settings** — the host, shared by both devices:

| Tab | What's in it |
|---|---|
| **Quest** | The optional [Quest-over-USB](#quest-usb) controller-pose link. |
...
| **System** | Log level. (The active-device selection is also stored here, but is edited with **Select** on the Axol / Mantis tiles.) |
...
Values are stored **on the Axol host** (`~/.almond/settings.json`) — not in your browser — so they're shared by every operation and every operator device, and they're folded into each operation start automatically (a per-run field always wins over a shared one). The same file is read by default by the [`axol` CLI commands](/cli/configuration#the-shared-settings-file) and the [SDK constructors](/api/robot#shared-settings), so a value saved here is what `axol teleop` and `Axol()` run too. Edits stage locally until you press **Save** (or **Discard**). Unset fields show the config's own default as a placeholder; edited ones get a reset button. **Export** / **Import** in the card footer move the whole settings file as JSON for backup or copying between robots.

Every setting on these tabs — and every field under **Advanced** — is documented with its default and meaning on each operation's **Config Parameters** page (e.g. [Teleoperation](/operations/teleop-config)). The [Settings File](/guides/control-panel-settings) page describes the file's layout — a nested tree whose `axol` / `teleop` / `kinematics` sections are the config dataclasses themselves — which is what you're editing in an exported `settings.json`.
...
<Note>
  ...
  cache, `~/.cache/huggingface/lerobot` of the account that ran the installer)
  and are handed back to that account after every episode save, so the same
  datasets are visible from the panel and the CLI.
</Note>

Pick an operation from the selector. Each panel asks only for its **per-run inputs** — the dataset repo id and task for collect data, the policy path/type for run policy, the episode to replay — plus **Start**/**Stop**. Everything reusable lives in [Settings](#settings). Hover the **ⓘ** next to a field for its description, pulled from the CLI's own help. Some inputs come with a pick list the host builds when the panel opens (the datasets on disk under **repo id**; a package built on `almond-axol` can declare its own, such as the deployable model ids for its policy operations) — pick an entry or keep typing, the field stays free-form; if the list can't be fetched the panel says why beneath the form and the input still works. A few fields are **strict** — their value must come from the list (a package's task catalog, say): those render as a select with no free typing, **Start** stays blocked with *Pick &lt;field&gt; from the list* until an entry is chosen, and the host refuses a start with an off-list value. If a strict field's list can't be fetched, the field can't be set until it can.
...
|---|---|---|
| **Teleoperation** | Axol, Mantis, or sim | Drive Axol from Quest, or — with Mantis selected on its tile — validate a Mantis rig by driving its grippers straight from the rig triggers over CAN (no tracking, headset, or cameras). Toggle **sim** for the browser visualizer (Axol only). When [Jelly](/guides/vr-interface#jelly)'s wheel or lift adapter is attached the thumbsticks drive it too — nothing to enable; to drive *just* Jelly with the arms untouched (no Axol connection or motor check needed) switch **Axol arms** off in the Robot tab. See the [Mantis tab of the teleop guide](/operations/teleop#run-it). If Axol camera slots are assigned, feeds are relayed and mirrored in the [operator view](#operator-view). |
...
| **Collect data** | Axol or Mantis + cameras | Record Axol joint or Cartesian actions, or (with Mantis selected on its tile) Mantis Cartesian TCP actions. Production Mantis start is blocked without a ready source and approved factory or measured transforms. The **repo id** picker can resume host datasets; feeds and [episode controls](#episode-controls) are mirrored in the panel. See the [Mantis tab of the data-collection guide](/operations/data-collection#run-it). |
...
The **Start** button stays disabled until everything an operation needs is ready, and a "Before you can start" list spells out what's missing — e.g. *Connect Axol*, *Fix motor fault: R elbow — over temperature*, *Assign at least one camera*, *Identify left + right Lighthouse trackers*, *Fix Ultimate private shared-map AP setup*, or *Add an approved factory or measured tracker-to-TCP transform*. Fix tracker setup in **Mantis settings → Tracking** ([Mantis Tracking](/mantis/tracking)); transforms are a [file on the host](/cli/tracker#transform-file). Tracker setup only gates Mantis *collection* — Mantis teleop never tracks (it drives the grippers straight from the rig triggers) and starts on CAN alone. The panel refreshes the same readiness the operation start uses.
