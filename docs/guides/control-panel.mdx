...
For **Mantis** teleop or collection, press **Select** on the Mantis tile and set up its pose source under **Mantis settings → Tracking** — see [Mantis Hardware](/mantis/hardware) for the rigs and hub and [Mantis Tracking](/mantis/tracking) for the Quest / Lighthouse / Ultimate setup flows and the start gesture. The panel launches and owns the selected tracker bridge and its two trigger readers; for collection a missing binding, incomplete channel mapping, or backend failure is a visible blocker rather than an apparently running session with no input. Teleop needs only CAN: while the source is not set up it shows a **Grippers only** notice and drives the grippers straight from the rig triggers. Teleop enables the physical grippers immediately; collection keeps them motor-disabled between takes. Mantis is not available for Collect DAgger or Run Policy.
...
|---|---|---|
| **Teleoperation** | Axol, Mantis, sim, or cart-only | Drive Axol from Quest, or — with Mantis selected on its tile — validate a Mantis rig from Quest/Lighthouse/Ultimate. Toggle **sim** for the browser visualizer (Axol only). Mantis enables its two grippers and renders virtual arms; see the [Mantis tab of the teleop guide](/operations/teleop#run-it). If camera slots are assigned, feeds are relayed and mirrored in the [operator view](#operator-view). |
...
The **Start** button stays disabled until everything an operation needs is ready, and a "Before you can start" list spells out what's missing — e.g. *Connect Axol*, *Fix motor fault: R elbow — over temperature*, *Assign at least one camera*, *Identify left + right Lighthouse trackers*, *Fix Ultimate private shared-map AP setup*, or *Add an approved factory or measured tracker-to-TCP transform*. Fix tracker setup in **Mantis settings → Tracking** ([Mantis Tracking](/mantis/tracking)); transforms are a [file on the host](/cli/tracker#transform-file). Tracker setup only gates Mantis *collection* — Mantis teleop lists the same items as a **Grippers only** notice and starts without tracking. The panel refreshes the same readiness the operation start uses.
