...
| **Teleop & VR** | Teleop control rate, position and rotation multipliers, return-to-rest speed. |
...
| **System** | Log level. |
| **Advanced** | Every remaining per-operation field, flattened into plain sections (pick the operation, then a section like *axol* or *vr server*) with a search box across all of it. Runs of identical sub-configs — the per-joint gains — render as **one table per arm** (a row per joint, a column per parameter), and vector values like a link's CoM are entered as separate **x / y / z** fields, never raw arrays. Nothing is hidden behind collapsed groups. |
