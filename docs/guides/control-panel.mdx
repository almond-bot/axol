...
5. Under **Measured tracker→TCP transforms**, enter each side's measured position `x/y/z` in metres and quaternion `qx/qy/qz/qw`, then save. The editor shows the exact active `survive:<codename>`, `ultimate:<mac>`, or `quest:<profile>:grip` key and writes only that entry. It does not measure the mount geometry: obtain those constants from the mount CAD or a physical calibration and verify them on the bench. Built-in Tracker 3.0 and 11 mm origin-adjusted Ultimate candidates appear as status hints only and do not satisfy the production calibration gate.
