...
| **Collect data** | Axol + Cameras | Record VR teleop episodes to a LeRobot dataset with the cameras. The feeds are mirrored in the panel's [operator view](#operator-view) and the [episode controls](#episode-controls) can start and end takes, so a session can be driven with the headset off. |
...
| **Return to rest** | Only during a **Contact** hold: the guarded return between episodes met resistance and the arms are limp. Clear them by hand, then press this to replan the return from where they are. |
...
| **Return to rest** | Only during a **Contact** hold: the guarded return met resistance and the arms are limp. Clear them by hand, then press this (or **X** on the controller) to replan the return from where they are. |
