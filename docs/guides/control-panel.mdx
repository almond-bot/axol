...
For **Mantis** teleop or collection, choose **Quest**, **Lighthouse**, or **Ultimate** under **Settings → Teleop & VR → Mantis tracking** (default: Lighthouse). The per-run **Mantis** toggle remains in the operation card. Quest waits for the WebXR headset to connect and does not launch a bridge. Lighthouse and Ultimate launch the matching tracker bridge using that backend's left/right bindings and the trigger CAN channels saved by [`tracker.identify`](/cli/tracker). The bridge starts before the VR server, reconnects until it is available, and is stopped automatically with the operation. While running, the panel shows **Toggle tracking** and **Reset**, replacing the bridge terminal's Enter and `r` controls. A missing binding or tracker-backend startup failure is shown as an operation error instead of leaving an apparently running session with no poses.
