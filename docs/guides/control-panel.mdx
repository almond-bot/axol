...
| **Robot** | Left/right arm stiffness sliders, CAN channels, gripper torque limit and max speed (both grippers), gravity-comp damping and rate, the **contact-stop** thresholds (the always-on reset guard plus the opt-in teleop and policy watchdogs — each a switch that arms it at a suggested value), and the **Jelly** toggle (drives the [Jelly](/guides/vr-interface#jelly) from the headset thumbsticks in teleop and data collection). |
