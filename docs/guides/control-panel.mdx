...
- Pings all 16 motors once per second while idle, so the tile reflects live motor health (a green/red dot per joint, grouped **L** / **R** — hover a dot for that motor's status, temperature, and voltage).

**Motor faults are surfaced and block everything.** A motor that is unreachable or reporting an error status (over-temperature, over-current, stall, encoder error, …) turns its dot red, flips the tile to a red *N motor faults* state, and lists each fault by joint (e.g. *R elbow — over temperature (81°C)*). While any fault is active, every hardware operation's **Start** is blocked — both in the panel and by the server itself — since driving through a faulted motor risks the arm. Sim teleop stays available.
