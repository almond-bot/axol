...
Data collection and policy inference also need the ZED cameras attached to the host. Press **Configure** to open **Settings → Cameras**.
...
Almost nothing you configure is actually per-run — camera assignment, arm stiffness, teleop rate, recording fps and codec, the inference server address — so it all lives in one **Settings** dialog (the gear on the **Axol Host** tile, or the *…live in Settings* link under any operation), split into categories:

| Category | What's in it |
|---|---|
| **Cameras** | The camera setup described [above](#cameras). |
| **Robot** | Left/right arm stiffness sliders, CAN channels, gravity-comp damping and rate. |
| **Teleop & VR** | Teleop control rate and the controller position multiplier. |
...
| **System** | Log level. |
| **Advanced** | The full per-operation config tree (searchable) for anything not covered above — per-joint gains, IK weights, VR server ports, … |

Values are stored **on the Axol host** (`~/.almond/settings.json`) — not in your browser — so they're shared by every operation and every operator device, and they're folded into each operation start automatically (a per-run field always wins over a shared one). Unset fields show the config's own default as a placeholder; edited ones get a reset button. **Export** / **Import** in the dialog footer move the whole settings file as JSON for backup or copying between robots.
...
The **Start** button stays disabled until everything an operation needs is ready, and a "Before you can start" list spells out what's missing — e.g. *Connect Axol*, *Assign at least one camera serial in Settings → Cameras*, or *Set &lt;required field&gt;*.
