...
Every arm-motion operation also carries an **rt** toggle (`robot config → rt` on the LeRobot-based ones): it drives the wire through the [`axol-rt` Rust realtime core](/cli/rt-install) instead of the Python control loop — the core owns CAN at 240 Hz while the operation streams targets. Experimental; same switch as [`teleop --rt`](/cli/teleop).
