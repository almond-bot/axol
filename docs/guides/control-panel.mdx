...
| **Mantis** | Quest/Lighthouse/Ultimate step-by-step setup flow (each step gated on host-reported readiness), logical left/right CAN mapping, and Left Mantis / Right Mantis cameras. Ultimate's protected private shared-map AP configuration can be saved without exposing the stored password. A live Quest run reports each controller's WebXR profile/space and a copyable common calibration key; use it as **Quest calibration key** so teleop and collection use the same selected datum. The measured-transform editor writes each side under its current bound device/profile key in `~/.almond/mantis/tcp_transform.json`. |
...
Everything that can safely run on the Linux/Jetson host is available under **Settings → Mantis** as a step-by-step flow. Select the source, then work through one step at a time: each step resolves from what the host actually reports (not from ticking a box), **Continue** only unlocks once it is resolved, and a step that regresses later pulls the flow back to it. Every fix is an action inside the step, so the terminal is not needed; the flow stays on the step whose action is running until it finishes.
...
| **Identify sides** | **Identify trackers**: move only the requested Mantis during each capture. Refuses to bind while base stations share a channel. | Same, once the dongle, Wi-Fi, and runtime steps are resolved. | — (WebXR handedness) |
| **Gripper offset** | Resolved automatically by the built-in factory transform for the flat-back mount; the editor only appears when a saved override is blocking or replacing it. | Same (the factory value includes the 11 mm origin adjustment). | Enter the bench-measured transform for both controllers under the exact `quest:<profile>:grip` key. |
| **Test** | Short Mantis teleop acceptance run and the Diagnostics trigger/gripper test. | Same, plus an optional **Run readiness check** (non-invasive; does not open the dongle or prove live poses). | Same. |

The equivalent terminal commands, manual JSON fallback, transform schema, and acceptance checks are in the [Mantis tracker CLI guide](/cli/tracker).
