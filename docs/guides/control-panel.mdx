...
The panel connects the selected hardware **automatically** once the host comes online (one attempt per profile, so a manual disconnect isn't immediately undone). Axol is the default; enabling **Mantis** for the selected run switches the idle telemetry link to the Mantis grippers automatically, and disabling it switches back to Axol. Each tile's **Connect** button is the manual fallback / reconnect. Bringing up the hardware:
...
5. Under **Measured tracker→TCP transforms**, enter each side's measured position `x/y/z` in metres and quaternion `qx/qy/qz/qw`, then save. The editor shows the exact active `survive:<codename>`, `ultimate:<mac>`, or `quest:<profile>:grip` key and writes only that entry. Ultimate entries also record the active quaternion-order/up-axis convention. If either convention changes—or an older entry has no convention proof—the editor marks the saved value stale and production collection stays blocked until you re-check the physical overlay and explicitly resave it. The editor does not measure the mount geometry: obtain those constants from the mount CAD or a physical calibration and verify them on the bench. Built-in Tracker 3.0 and 11 mm origin-adjusted Ultimate candidates appear as status hints only and do not satisfy the production calibration gate.
...
| **Collect data** | Axol or Mantis + cameras | Record Axol joint actions or Mantis Cartesian TCP actions. Production Mantis start is blocked without a ready source and measured/verified transforms. The **repo id** picker can resume host datasets; feeds and [episode controls](#episode-controls) are mirrored in the panel. See the [Mantis collection flow](/operations/data-collection#mantis-collection-end-to-end). |
