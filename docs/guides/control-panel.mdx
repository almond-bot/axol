...
It's served by [`axol serve`](/cli/serve) and runs each operation through the same entry points as the CLI commands (teleop and gravity-comp in their own subprocess, the rest in-process), so it's a drop-in alternative to the per-command CLI workflow.
