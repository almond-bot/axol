...
| **Robot** | Left/right arm stiffness sliders, CAN channels, per-gripper torque limit and max speed, gravity-comp damping and rate, the **contact-stop** thresholds (the always-on reset guard plus the opt-in teleop and policy watchdogs — each a switch that arms it at a suggested value), and the **Jelly** toggle (drives the [Jelly](/guides/vr-interface#jelly) from the headset thumbsticks in teleop and data collection). |
...
|---|---|---|
| **Teleoperation** | Axol, Mantis, sim, or Jelly-only | Drive Axol from Quest, or — with Mantis selected on its tile — validate a Mantis rig by driving its grippers straight from the rig triggers over CAN (no tracking, headset, or cameras). Toggle **sim** for the browser visualizer (Axol only). If the robot has [Jelly](/guides/vr-interface#jelly), the **Jelly** setting (Robot tab) also lets the thumbsticks drive it; tick **Jelly only** to drive *just* Jelly with the arms untouched — no Axol connection or motor check needed. See the [Mantis tab of the teleop guide](/operations/teleop#run-it). If Axol camera slots are assigned, feeds are relayed and mirrored in the [operator view](#operator-view). |
