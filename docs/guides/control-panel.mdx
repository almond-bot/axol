...
Pick an operation from the selector. Each panel asks only for its **per-run inputs** — the dataset repo id and task for collect data, the policy path/type for run policy (policy type `custom` runs [your own model](/operations/custom-policy), where the path becomes optional), the episode to replay — plus **Start**/**Stop**. Everything reusable lives in [Settings](#settings). Hover the **ⓘ** next to a field for its description, pulled from the CLI's own help. Some inputs come with a pick list the host builds when the panel opens (the datasets on disk under **repo id**; a package built on `almond-axol` can declare its own, such as the deployable model ids for its policy operations) — pick an entry or keep typing, the field stays free-form; if the list can't be fetched the panel says why beneath the form and the input still works.
