...
- Pings all 16 motors once per second while idle, so the tile reflects live motor health as one row of squares grouped **L** / **R**. Each square combines CAN reachability and motor error status; hover it for details and temperature.
...
For **Mantis** teleop or collection, choose **Quest**, **Lighthouse**, or **Ultimate** under **Settings → Mantis** (default: Lighthouse). The per-run **Mantis** toggle remains in the operation card. Quest waits for the WebXR headset to connect and does not launch a bridge. Lighthouse and Ultimate show whether both sides are bound in the same settings tab; **Identify trackers** guides you through moving the left and right Mantis and saves the binding without a terminal. They then launch the matching tracker bridge using that binding and the trigger CAN channels. The bridge starts before the VR server, reconnects until it is available, and is stopped automatically with the operation. Plain teleop and data collection engage automatically once both trackers are live. There is no manual pause state: **Reset** remains available, and tracking resumes automatically afterward. If either tracker loses lock, motion freezes until both are fresh, then re-anchors and resumes automatically. Teleop enables the physical grippers immediately; collection keeps them motor-disabled between takes and force-disables them again after every save, rejection, stop, or error. Mantis is not available for Collect DAgger or Run Policy; those operations always use Axol. A missing binding or tracker-backend startup failure points back to the binding control instead of leaving an apparently running session with no poses.
...
The Cameras tab **detects the cameras** plugged into the host and shows one card per camera: its serial, its type (**stereo** ZED X / **mono** ZED-X One — detected automatically, never flagged manually), the model, and a **live preview frame**, so you can tell at a glance which physical camera each serial belongs to. Choose each camera from its slot's dropdown — an assigned slot repeats the camera's preview thumbnail next to its name. With Mantis off, the tab shows the Axol layout (overhead / left arm / right arm); with the operation's **Mantis** toggle on, it shows the separately stored two-camera layout (**Left Mantis** / **Right Mantis**). Changing the Mantis cameras does not overwrite the Axol mapping. Press **Refresh** to re-query the daemon's device list and grab fresh preview frames, or **Restart daemon** to restart the ZED X daemon so it re-scans the GMSL links — needed to pick up a camera plugged in or unplugged since boot. (Previews are unavailable while an operation is running, since it owns the cameras.)
