...
For **Mantis** teleop or collection, Start also launches the tracker bridge using the backend, left/right bindings, and trigger CAN channels saved by [`tracker.identify`](/cli/tracker). The bridge starts before the VR server, reconnects until it is available, and is stopped automatically with the operation. While running, the panel shows **Toggle tracking** and **Reset**, replacing the bridge terminal's Enter and `r` controls. A missing binding or tracker-backend startup failure is shown as an operation error instead of leaving an apparently running session with no poses.
