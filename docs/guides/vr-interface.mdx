...
| — | Left thumbstick | **Drive Jelly** — push to move forward/back and strafe (robots with [Jelly](#powered-cart) only; deadman, so the base stops when released) |
...
Enable Jelly per robot from the control panel's **Jelly** toggle (Robot settings) or with [`axol teleop --cart.enabled true`](/cli/teleop); the tunables live under **Advanced → Jelly** (the compatibility `--cart.*` flags). To drive **only** Jelly with the arms left untouched — no Axol hub needed — use the control panel's **Jelly only** checkbox or [`axol teleop --cart_only`](/cli/teleop). See [`can.setup`](/cli/can-setup) for wiring Jelly's CAN adapter.
