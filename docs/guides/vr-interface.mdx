...
The headset only offers the views for cameras that are actually being streamed (set in the [control panel's Cameras dialog](/guides/control-panel#cameras) or via the relevant CLI flags). A flick toward a camera that isn't streamed is ignored, so streaming only the overhead, say, leaves the default passthrough view and the overhead view and nothing else. While video is still negotiating — or any streamed camera hasn't produced its first frame — a global **Connecting cameras…** indicator shows regardless of the current view; it clears once every streamed camera is live, and never appears when nothing is being streamed.
...
| — | Right thumbstick (flick) | **Switch camera view** — see [Camera views](#camera-views) |
