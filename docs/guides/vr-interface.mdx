...
**Box mode** is for carrying something with both hands: the grippers face each other like two palms on a box and **one controller drives both arms as a rigid pair**. Toggle it with the **Box** button on the HUD tool row, by **clicking both thumbsticks together**, from the control panel's live [session settings](#session-settings), or start sessions in it with the control panel's **Box mode at startup** setting (`--teleop.box_mode`). The server owns the mode and echoes it back to every client, so the HUD toggle always shows the live state; switching modes disengages the arms.

The natural moment to switch is once you've brought the grippers into the grasp with two-handed teleop: the **Box** button reads **Box: OFF (aligned)** (in blue) while the two grippers already face each other across a box-sized gap — the IK worker checks the pair geometry a few times a second — meaning the engage blend will barely move them. Click both sticks, grip once, and lead.

While box mode is on:

| Control | Action |
|---|---|
| Either grip | Press once to **engage**. The arms first blend into the parallel pair — same midpoint and width they have now, grippers facing each other — over `box_align_duration` (1.5 s), then the pressed controller **leads**: its motion moves the pair as one body (the usual clutch mapping, so only motion after the press counts). Press the *other* grip to hand over the lead to that controller; press the leading grip again to freeze. **X** returns to rest as usual |
| Leader's trigger | Closes / opens **both** grippers together |
| Leader's thumbstick | **Jog** the pair horizontally in its own frame: forward = away from the gripper-to-gripper line, left/right along it. Hold the stick **clicked** to jog **up/down** (y) and **yaw** the pair about its centre (x) instead |
| Other thumbstick | Jog the pair **up/down** (y) and change the **width** between the grippers (x, push right = wider; clamped to `box_width_min`..`box_width_max`) |

Jogging adds to the leader's motion, so you can hold the pair with one hand and nudge it with a stick. Jelly is inert in box mode — the thumbsticks belong to the arms. Speeds live under `--teleop.box_jog_speed` (m/s), `--teleop.box_jog_yaw_speed` (rad/s) and `--teleop.box_width_speed` (m/s).

**After a hand-off.** When a person has taken the object or moved the arms by hand, the grippers rarely end up in a pair — but a box-mode engage doesn't care where they are: its first step is always the blend into a parallel, facing pair at the arms' current midpoint and width, so just grip again and the leader takes over once they're aligned.
