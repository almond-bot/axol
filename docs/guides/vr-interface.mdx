...
<Warning>
  The WSS link is self-signed. Before the app can connect, accept the certificate once in the Quest browser by visiting `https://<hostname>.local:8000` or `https://<local-ip>:8000` and proceeding past the warning. The cert is cached in `~/.almond/vr/certs/` and shared with [`axol serve`](/cli/serve).
</Warning>
