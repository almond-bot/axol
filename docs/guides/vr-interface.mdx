...
| 6 | A | **Record**: start a take (3-second countdown), or stop and **save** the current take — **data collection only** (no effect during plain teleop) |
| 7 | Y | **Exit** the XR session |
| — | Right thumbstick (click) | **Reset the camera screens** — re-anchor to the current gaze and clear moves + resizes |
