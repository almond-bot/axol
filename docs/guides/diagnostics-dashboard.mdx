...
Each diagnostic — [`diag.rom-enable`](https://docs.almond.bot/cli/diag-rom-enable), [`diag.rom-disable`](https://docs.almond.bot/cli/diag-rom-disable), and the [`diag.zed-cable`](https://docs.almond.bot/cli/diag-zed-cable) camera-cable check — is a card: click it, set parameters in the dialog, and hit Run. Axol ROM tests expose a joint picker; Mantis ROM tests automatically run the gripper-only path. The card shows the running state, elapsed time, and latest output; completion lands as a notification and the full log is kept below. CAN-owning tests pause idle telemetry for their duration.
