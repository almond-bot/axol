...
Before setting up cameras or tracking, prove that each trigger drives its own gripper. Mantis teleop needs only CAN for this: with Mantis selected, pick **Teleoperation** and press **Start**. While the selected tracking source is not set up, the panel shows a **Grippers only** notice and the run drives the grippers straight from the rig triggers — no cameras, headset, trackers, or transforms. From the CLI:

```bash
axol teleop --mantis                        # grippers only until tracking is set up
axol teleop --mantis --mantis_grippers_only true   # skip tracking even when it is
```
