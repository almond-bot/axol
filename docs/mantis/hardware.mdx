...
The grippers are driven by the same `axol-rt` realtime core as the robot's arms (see [Provision](/cli/provision)): for the duration of each take — and for a Mantis teleop run — the core owns the two gripper CAN interfaces and streams the POSITION_FORCE commands at 240 Hz, while Python only reads the triggers and trackers and hands it normalised targets. Between takes the grippers are torque-off and the core is stopped; the one-time open-stop calibration and the torque-off checks run from Python on the idle bus. The trigger reader's receive-only socket shares each interface with the core (the kernel duplicates frames to every open socket, and the core ignores the trigger node's ID).
...
<Note>
  `axol can.setup` is a disruptive link reset for every attached adapter (Axol arms, Jelly, and Mantis pair), so run it while nothing is driving the robot.
</Note>
