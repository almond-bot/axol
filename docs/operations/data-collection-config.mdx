...
In the [shared settings file](/guides/control-panel-settings) (`~/.almond/settings.json`, read by default) the same sections appear without this command's wrapper prefixes: `robot_config.axol_config.*` is the file's `axol`, `teleop_config.vr_teleop_config.*` is `teleop`, `teleop_config.kinematics_config.*` is `kinematics`, `teleop_config.vr_server_config.*` is `vr_server`, `teleop_config.jelly.*` is `jelly`, the other `robot_config.*` fields are `lerobot`, and `fps` / `root` / `vcodec` / `push_to_hub` / `rerun_*` live under `recording`.
...
| `--fps` | `30` | Recording rate. Cameras capture at 60 and are decimated to this before encoding; 60 doubles the dataset encode/recorder load on the Jetson. |
