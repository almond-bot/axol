...
Data collection records teleoperation episodes plus synchronized camera frames into a [LeRobot](/api/lerobot) dataset you can later train on. Axol records joint trajectories; **Mantis** records Cartesian TCP poses from Quest, Lighthouse, or Ultimate tracking. Everything runs on **one machine**: the computer wired to the hardware with the ZED cameras attached (typically a ZED Box / Jetson), so images and actions share one clock.
...
- **The selected pose source set up.** Quest needs the VR TLS certificate authorized once. Lighthouse/Ultimate need their Linux runtime, two identified devices, and live trigger nodes. Production Mantis collection also requires measured or bench-verified tracker→TCP transforms for both sides.
...
Complete the matching [Mantis teleop acceptance](/operations/teleop#mantis-teleop-end-to-end) before creating training data. A CAD candidate, legacy unkeyed offset, generic absolute-mode sim, or start-pose-only alignment is not production calibration.
  ...
  <Step title="Bring up the selected pose source">
    <Tabs>
      ...
      <Tab title="Lighthouse">
        Select **Lighthouse** after installing libsurvive, pairing both VIVE Tracker 3.0 units, and completing **Identify trackers** in Settings → Mantis. Save the measured mounts in that tab's transform editor under the displayed `survive:&lt;codename&gt;` keys; the Tracker 3.0 CAD candidate does not count. Start and wait for two fresh tracker poses **and** two trigger heartbeats. At the start pose, release both rig triggers and squeeze them together once to align/engage.
      </Tab>

      <Tab title="Ultimate">
        Select **Ultimate** after the one-time Windows pair/map flow. In Settings → Mantis, install the runtime, save the private shared-map AP configuration (not the Windows or robot LAN), physically connect/power the dongle and trackers, complete guided **Identify trackers**, save the measured mounts under the displayed `ultimate:&lt;mac&gt;` keys, and choose **Run readiness check**. The bundled 11 mm origin-adjusted transform remains a candidate and does not satisfy the production calibration gate. Windows VIVE pairing/map creation and the requested physical movement remain operator steps; the rest is available in the panel. Start only with both trackers localized in the shared map and both trigger nodes live. At the start pose, release both rig triggers and squeeze them together once. See the [UI and CLI setup routes](/cli/tracker#vive-ultimate-tracker-setup-dongle--pyvut).
      </Tab>
    </Tabs>
  </Step>
