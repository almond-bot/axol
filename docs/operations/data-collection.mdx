...
<Steps>
  <Step title="Select Mantis and assign its cameras">
    In the control panel press **Select** on the **Mantis** tile, choose **Collect Data**, then click the Mantis tile to open **Mantis settings**. Select and save the pose source under **Tracking**, verify the two distinct CAN channel fields under **CAN**, and assign **Left Mantis / Right Mantis** cameras under **Cameras**. Those two slots are stored separately from the overhead / left arm / right arm layout in **Axol settings → Cameras**. At least one Mantis camera must have recording enabled.
