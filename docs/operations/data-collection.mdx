...
<Steps>
  <Step title="Select Mantis and assign its cameras">
    In the control panel choose **Collect Data**, enable **Mantis**, then open **Settings → Mantis**. Select and save the pose source, verify the two distinct CAN channel fields, and assign **Left Mantis / Right Mantis** cameras. Those two slots are stored separately from the overhead / left arm / right arm layout in **Settings → Axol Cameras**. At least one Mantis camera must have recording enabled.
