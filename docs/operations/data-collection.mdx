...
<Steps>
  ...
  <Step title="The arms return home — guarded">
    After a take ends — saved or discarded — the arms head back to the rest pose on their own, but **gently**: the move runs at the softest gains the arms are tuned for, and a torque watchdog compares every joint against the gravity model as it goes.
...
<Note>
  **Crash-safe resume.** Collection refuses to resume a dataset whose saved episodes don't form a contiguous `0…N-1` range — the tell-tale of a recorder killed mid-save, where buffered episodes were lost while the episode counter had already advanced. Recording past such a gap would silently misalign every later episode with a different episode's video, so it stops with guidance (renumber the survivors to be contiguous, or start a fresh dataset) instead. Keeping the host on a current release matters here too: the service now gives a recording session up to ~200 s to finalize when it's stopped, so a shutdown mid-save doesn't drop the take in the first place — [re-run the installer or press **Update**](/installation#staying-up-to-date) to pick this up on an older station.
</Note>
