...
<Tabs>
  <Tab title="Control Panel">
    <Steps>
      ...
      <Step title="Select Gravity Compensation and Start">
        ...
        To record the hand-guided session, set a **recording name** on the panel before starting — the capture is written when the session stops, and **Build motion** in the diagnostics dashboard turns it into a replayable [reference motion](/cli/motion-build).
      </Step>
    </Steps>
  </Tab>
  ...
  <Tab title="CLI">
    ...
    axol gravity-comp --record demo1                  # capture the hand-guided motion
    ```

    Press `Ctrl+C` to stop. Use `--free_joints` to float only the named joints while the rest of the arm holds its pose — handy for isolating one joint at a time. `--record` captures the hand-guided session so [`motion.build`](/cli/motion-build) can turn it into a replayable reference motion. See the [`gravity-comp`](/cli/gravity-comp) reference for every flag.
  </Tab>
</Tabs>
...
<CardGroup cols={2}>
  <Card title="Config Parameters" icon="sliders" href="/operations/gravity-comp-config">
    Every setting for gravity comp, with its default.
  </Card>
