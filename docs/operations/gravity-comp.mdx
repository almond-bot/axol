...
<Tabs>
  <Tab title="Control Panel">
    {/* SCREENSHOT: control-panel-gravity-comp.png — the Gravity Compensation operation selected with the Start button */}

    <Steps>
      <Step title="Connect the robot">
        Connect the **Axol Host** and press **Connect** on the **Axol** tile. Gravity comp needs only the robot connection — no cameras. See the [Web Control Panel guide](/guides/control-panel#connections).
      </Step>
