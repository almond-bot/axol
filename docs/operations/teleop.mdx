...
<Steps>
  <Step title="Connect the Mantis hardware and map its sides">
    ...
    Camera layouts are separate: **Axol settings → Cameras** holds overhead / left arm / right arm, while **Mantis settings → Cameras** contains the two-camera **Left Mantis / Right Mantis** layout. Save after changing the source, channels, or cameras.
  </Step>

  <Step title="Prepare and select one pose source">
    <Tabs>
      <Tab title="Quest">
        Select **Quest** and save. Start the run, open [axol.almond.bot](https://axol.almond.bot) in the headset, connect to the host, authorize its certificate if prompted, and enter VR. WebXR handedness identifies left/right automatically; do not run tracker pairing or Identify. During the live run, Settings → Mantis shows both reported profile/space values and a copyable common key. Use it as **Quest calibration key**, then enter both measured mounts in `~/.almond/mantis/tcp_transform.json` under that identical `quest:&lt;reported-profile&gt;:grip` key. See [Meta Quest setup and the schema](/cli/tracker#meta-quest-setup).
      </Tab>

      <Tab title="Lighthouse">
        Select **Lighthouse** and save. `axol provision` normally installs the runtime; if readiness is red, use **Install Lighthouse support** in this tab (the CLI equivalent is `axol tracker.install`). Power the base stations, pair each VIVE Tracker 3.0 with its Watchman dongle, then use **Pair tracker** and **Identify trackers** here. Move only the requested rig during each Identify capture. Both sides must be bound, visible, and reporting fresh poses before the run becomes ready. The standard flat-back mount uses the built-in factory transform, so setup ends at Identify; a per-unit override, if ever needed, is written to `~/.almond/mantis/tcp_transform.json` from the CLI.
      </Tab>

      <Tab title="Ultimate">
        Select **Ultimate** and save. First pair both VIVE Ultimate trackers to the dongle and create/refine their shared tracking map in VIVE Hub on **Windows**; HTC's Windows-only service and physical room scan cannot be automated by the Jetson panel. Back in Settings → Mantis, use **Install Ultimate support**, save the protected shared-map AP configuration, connect and power the dongle/trackers, and run **Identify trackers**; the flow ends once both sides are bound (`axol tracker.ultimate.check` is the optional non-invasive host check). The standard flat-back mount automatically uses the factory transform `[0, 0.0465, -0.092, 0.7071068, 0, 0, 0.7071068]`; its axes match Tracker 3.0 and require no extra rotation. Overrides are managed from the CLI, not the panel. Changing from the pinned `wxyz` / z-up convention disables the factory fallback until an override is checked and saved for that convention. See [VIVE Ultimate setup](/cli/tracker#vive-ultimate-tracker-setup-dongle--pyvut).
      </Tab>
    </Tabs>
  </Step>

  <Step title="Start, align, and test recovery">
    For Quest, hold the rigs at the agreed start pose, release both controller grip buttons, then press both grips together to align/engage. For managed Lighthouse/Ultimate, wait for **both tracker poses and both trigger heartbeats**, hold the rigs at the start pose, then **squeeze both rig triggers together and release both**; the world→base transform is fitted at the rigs' pose on that release. The start gesture is consumed and does not count toward an episode gesture.

    If either pose source drops, both sides freeze. Recovery is deliberately inert: restore both sides, then repeat the start gesture (Quest: release then press both grips; Lighthouse/Ultimate: squeeze both triggers then release). Reset returns to rest and requires the same fresh gesture. It never re-anchors and moves automatically.
  </Step>
