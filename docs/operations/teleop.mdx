  ...
  <Step title="Prepare and select one pose source">
    <Tabs>
      ...
      <Tab title="Lighthouse">
        Select **Lighthouse** and save. `axol provision` normally installs the runtime; if readiness is red, use **Install Lighthouse support** in this tab (the CLI equivalent is `axol tracker.install`). Power the base stations, pair each VIVE Tracker 3.0 with its Watchman dongle, then use **Pair tracker** and **Identify trackers** here. Move only the requested rig during each Identify capture. Both sides must be bound, visible, and reporting fresh poses before the run becomes ready. The bundled transform is currently a CAD candidate, not a production-verified constant; enter the measured values in the transform editor shown under `survive:&lt;left-codename&gt;` / `survive:&lt;right-codename&gt;`, then complete acceptance below.
      </Tab>

      <Tab title="Ultimate">
        Select **Ultimate** and save. First pair both VIVE Ultimate trackers to the dongle and create/refine their shared tracking map in VIVE Hub on **Windows**; HTC's Windows-only service and the physical room scan cannot be automated by the Jetson panel. Back in Settings → Mantis, use **Install Ultimate support**, save the protected configuration/fallback in the **Private shared-map AP** editor (not the Windows or robot LAN), then physically connect the dongle and power both trackers. In USB-dongle mode the tracker firmware supplies the active host AP credentials; the setup page explains how the saved fallback is used. Use **Identify trackers**, moving only the requested rig, enter the measured mounts under the displayed `ultimate:&lt;left-mac&gt;` / `ultimate:&lt;right-mac&gt;` keys, and choose **Run readiness check**. Each saved mount is bound to the active Ultimate quaternion-order/up-axis convention; changing that convention makes it stale and blocks collection until a bench re-check and explicit resave. The bundled 11 mm origin-adjusted transform is a candidate only and does not replace those measured entries. The [VIVE Ultimate setup](/cli/tracker#vive-ultimate-tracker-setup-dongle--pyvut) includes the equivalent CLI route and file schemas.
      </Tab>
    </Tabs>
  </Step>
