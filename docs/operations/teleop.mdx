...
<Note>
  On a robot with **Jelly**, the headset thumbsticks drive Jelly's base and lift during teleop — see [Jelly](/guides/vr-interface#jelly). Nothing to enable: Jelly is inferred from the CAN devices attached, and the Robot tab's **Jelly wheels** / **Jelly lift** switches (or [`--jelly.wheels false` / `--jelly.lift false`](/cli/teleop)) opt attached hardware out. Drive just Jelly with the arms left untouched by switching **Axol arms** off (or [`axol teleop --arms false`](/cli/teleop)).
</Note>
...
<CardGroup cols={2}>
  <Card title="Config Parameters" icon="sliders" href="/operations/teleop-config">
    Every setting for teleop, with its default.
  </Card>
