---
title: "Teleoperation"
description: "Drive Axol, or run the Mantis rigs — grippers only over CAN, or tracked by Quest, Lighthouse, or Ultimate."
---

Teleoperation drives the arms live from a Meta Quest headset. It runs on either device: **Axol** (the robot, or the browser **sim**) or **Mantis** (the handheld rigs — used to bring up the grippers over CAN alone, then to validate the Quest, Lighthouse, or Ultimate pose source before collecting data). You can run either from the **web control panel** or the **CLI** (`axol teleop`).
...
<Tabs>
  ...
  <Tab title="Mantis">
    Mantis teleop enables only the two physical grippers; the Axol arms exist as an IK-driven URDF overlay. It needs only CAN to start: while the selected Lighthouse/Ultimate source is not set up, the run is **grippers only** — the rig triggers drive the grippers and no tracking, headset, or cameras are used. Once tracking is set up, the same run proves the exact source, side mapping, mount transform, and loss/recovery behavior that collection will use — the [acceptance checklist](/mantis/tracking#acceptance) is the goal.
    ...
    <Steps>
      ...
      <Step title="Start grippers only (first bring-up)">
        With Mantis selected only **Teleoperation** and **Collect data** are listed. Pick **Teleoperation** (there is no sim or cart option on Mantis). Before tracking is set up the panel shows **Grippers only** with the tracking steps still to do; press **Start** anyway. Once both triggers read live and released, the grippers open to their hard stops (keep them clear), then each trigger closes its gripper proportionally. Squeeze the physical **left** trigger: if the **right** gripper moves, swap the CAN side mapping. Only a faulted gripper or a stale trigger heartbeat blocks this run.
      </Step>

      <Step title="Set up the pose source">
        Click the Mantis tile's title to open **Mantis settings → Tracking** and finish the selected source's setup flow (Lighthouse/Ultimate: sides identified; Quest: calibration key saved) — [Mantis Tracking](/mantis/tracking) walks through each source. Wrist cameras, if any, are assigned under **Mantis settings → Cameras**. With the flow complete the **Grippers only** notice disappears and the next **Start** owns the tracker bridge and both trigger readers.
      </Step>
    ...
    **From the CLI**, the same run owns the selected bridge (the saved source is used when `--mantis_source` is omitted), and falls back to grippers only when that source is not set up:
    ...
    axol teleop --mantis --mantis_grippers_only true   # triggers → grippers, no tracking
    ```
