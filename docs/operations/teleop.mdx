...
<Note>
  On a robot with **Jelly**, the headset thumbsticks drive Jelly's base and lift during teleop — see [Jelly](/guides/vr-interface#powered-cart). Enable it with the control panel's **Jelly** toggle or [`axol teleop --cart.enabled true`](/cli/teleop), or drive just Jelly with the arms left untouched using **Jelly only** / [`axol teleop --cart_only`](/cli/teleop).
</Note>
