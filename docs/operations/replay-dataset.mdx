...
Replay plays one episode of an existing [LeRobot](/api/lerobot) dataset straight back onto the arms: the episode's recorded `action` column is streamed to the motors frame by frame; on a clean stop the arms are then parked — eased to the rest pose and down to the all-zeros pose — before the motors release (after an error, or with `--park_on_exit false`, there is no teardown motion). It's the inverse of [data collection](/operations/data-collection) — instead of recording teleop actions it reproduces an already-recorded take on the hardware. You can launch it from the **web control panel** or the **CLI** (`axol replay-dataset`).
...
<Tabs>
  <Tab title="Control Panel">
    <Steps>
      ...
      <Step title="Start">
        Clear the workspace, then press **Start**. The arm moves to rest, replays the episode at the recorded timing, and is parked — eased to the rest pose, then down to the zero pose — before the motors release. With **loop** on it repeats (returning to rest between takes) until you press **Stop**, which parks the arms the same way.
      </Step>
    </Steps>
  </Tab>
  ...
  <Tab title="CLI">
    ...
    `--fps 0` (default) replays at the dataset's recorded fps, reproducing the original timing; set a positive value to override it. Match the stiffness used at collection time with `--robot_config.axol_config.left_stiffness` / `right_stiffness` so the arm tracks the recorded actions the same way. Press `Ctrl+C` to stop — on a clean stop the arms are parked (rest pose, then zero) before the motors release; a second `Ctrl+C` aborts the park, and `--park_on_exit false` skips it entirely. See [`replay-dataset`](/cli/replay-dataset) for the full field list and [Command configuration](/cli/configuration) for the override syntax.
  </Tab>
</Tabs>
