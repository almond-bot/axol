...
- **For local GPU inference**, the JAX CUDA plugin — installed by `axol jax.install` (run automatically by `axol provision`), which detects this device's CUDA version and installs the matching `jax[cudaNN]`. Or offload to a remote server (below).
  ...
  <Tab title="Control Panel">
    <Steps>
      <Step title="Connect the robot and assign cameras">
        Connect the **Axol Host** and **Axol**, then assign the cameras the policy was trained on in the **Cameras** dialog (at least one). See [Cameras](/guides/control-panel#cameras).
      </Step>
...
<Steps>
  <Step title="Start the inference server on the GPU machine">
    The inference server runs on the GPU machine from the CLI — it isn't one of the control panel's operations. With the `lerobot` extra installed and the JAX CUDA plugin provisioned (`axol jax.install`) there:
