...
<Tip>
  Not a LeRobot checkpoint? Pick policy type **`custom`** to drive the arms from your own model: it receives joints + camera frames and returns action chunks through the [`almond_axol.policy`](/api/policy) SDK. See [Run Your Own Policy](/operations/custom-policy).
</Tip>
...
- **A trusted trained checkpoint and any existing rollout dataset you append to** that you created or obtained from a source you trust (local path or HuggingFace repo), plus the checkpoint's policy type (`act`, `smolvla`, `pi0`, …). Upstream model deserialization and dataset parsing are not sandboxed. Assign the **same** cameras, and use the **same** camera resolution, stereo setting, and fps the policy was trained on.
  ...
  <Tab title="Control Panel">
    <Steps>
      <Step title="Connect the robot and assign cameras">
        Connect the **Axol Host** and **Axol**, then assign the cameras the policy was trained on under **Axol settings → Cameras** (at least one). See [Cameras](/guides/control-panel#cameras).
      </Step>
...
<Steps>
  <Step title="Start the inference server on the GPU machine">
    ...
    Replace the example with the inference machine's exact LAN IP, or omit `--host` to use the default `0.0.0.0` (all interfaces). It listens on port `8765` until `Ctrl+C`. See [`inference-server`](/cli/inference-server).
...
<CardGroup cols={2}>
  <Card title="Config Parameters" icon="sliders" href="/operations/run-policy-config">
    Every setting for run policy, with its default.
  </Card>
  ...
  <Card title="Run Your Own Policy" icon="code" href="/operations/custom-policy">
    Drive the arms from a model that isn't a LeRobot checkpoint.
  </Card>
</CardGroup>
