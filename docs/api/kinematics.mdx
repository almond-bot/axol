...
Bimanual inverse kinematics using pyroki and JAX/jaxls. Loads the bundled URDF, builds a collision model (only with `self_collision=True`; base collision detection is off by default), and JIT-compiles the solver during `__init__` — the first solver in a process takes a few seconds; subsequent calls (and subsequent solver instances) are fast.
...
| `self_collision` | `False` | Whether the arm↔torso (`base` / `s1`) collision model is part of the solve at all. Off by default: no capsule model is built, the self-collision cost is left out of the IK graph, the hard upper-arm/base guard on the final step is skipped, and the return-to-rest planner interpolates in joint space with joint limits only (limits are still clamped every step). Set `True` to restore the collision-aware solve the two fields below tune |
