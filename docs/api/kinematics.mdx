...
Bimanual inverse kinematics using pyroki and JAX/jaxls. Loads the bundled URDF, builds the arm↔torso collision model (`self_collision`, on by default), and JIT-compiles the solver during `__init__` — both graph shapes, with and without elbow hints, so box mode's first frame never waits on a compile; the first solver in a process takes a few seconds (a persistent on-disk cache makes later runs fast); subsequent calls (and subsequent solver instances) are fast.
...
| `ik(q_current, left_pose, right_pose, left_elbow_pos, right_elbow_pos)` | IK from the current joint vector; `pose` is `(pos_3, rot_3x3)`; elbow hints are optional `(3,)` arrays |
...
| `elbow_weight` | `0.0` | Elbow hint tracking weight for the headset's elbow. `0` (the default) disables it — the headset's inferred elbow proved unreliable on hardware, so in plain teleop the arm's swivel is left to the posture attractor. When enabled, the hint is projected onto the robot's reachable elbow sphere and fades out above shoulder height (see `elbow_fade_band`). Independent of this, box mode can pass its own synthetic "elbows out" hint with a per-call weight (`VRTeleopConfig.box_elbow_out` / `box_elbow_weight`, off by default, via `ik(elbow_weight=...)`, no fade) |
...
| `self_collision` | `True` | Whether the arm↔torso (`base` / `s1`) collision model is part of the solve. On by default: the capsule model is built, the soft self-collision cost is in the IK graph, the hard upper-arm/base guard runs on every final step so a pose target cannot push an arm into the torso, and the return-to-rest planner is collision-aware. Plain teleop rarely exercises it (the operator's own arms keep targets plausible); [box mode](/guides/vr-interface#box-mode) does, since its parallel, closing gripper poses would otherwise fold the elbows into the base. `False` drops all of it and the reset planner interpolates in joint space with joint limits only (limits are clamped every step either way). The two fields below tune the collision-aware solve |
| `self_collision_margin` | `0.025` m | Minimum clearance between collision bodies (with `self_collision` on). Kept below the arm–torso clearance of normal teleop poses so the collision cost isn't active during ordinary tracking |
| `self_collision_weight` | `150.0` | Self-collision penalty weight (with `self_collision` on) |
