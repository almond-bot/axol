...
`almond_axol.kinematics.path.plan_linear_segment(solver, q_from, q_to, ...)` returns the joint vectors that walk both grippers along a **straight world-frame line** between the poses of two joint configurations — position lerped, orientation slerped, every sample resolved with `ik()` and the posture attractor swept between the endpoints so the redundant elbow keeps the configuration you started from.

The whole segment is solved up front, at one vector per control tick, so the result can be streamed to `motion_control` verbatim. A sample that cannot be tracked within `pos_tolerance` raises `PathPlanningError` — nothing has moved at that point. This is what [`axol waypoints`](/cli/waypoints) replays; for a shortest-path move in joint space instead (returning to rest, for example), see `almond_axol.teleop.trajectory.plan_collision_aware_trajectory`.
