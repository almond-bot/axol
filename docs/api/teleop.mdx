...
| `teleop_max_vel` | `60/60/60/120/180/180/180 °/s` | Per-joint trapezoidal filter velocity caps (ARM_JOINTS order, both arms) — the per-tick command-step budget: 0.25°/tick shoulders, 0.5° elbow, 0.75° wrists at 240 Hz, the ceiling on how fast a solver jump / outlier / stall recovery can move each joint |
...
| `record` | `None` | File prefix for the teleop flight recorder; captures every pipeline stage to `<prefix>_{ik,cmd,meas}.npz` (latest engage→disengage segment, written at disengage) for `axol diag.teleop-jitter` |
