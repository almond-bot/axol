...
<Note>
  Requires LeRobot ≥ 0.6.1, Linux, and Python 3.13 on the robot machine; the `zed` camera additionally needs `pyzed` ([`zed.install`](/cli/zed-install)). See the [plugin README](https://github.com/almond-bot/axol/tree/main/plugins/lerobot_robot_axol) for adding it to an existing one-command-install `axol` environment.
</Note>
