...
<Warning>
  Treat policy checkpoints and existing LeRobot datasets as trusted input. The
  pinned upstream runtime parses dataset metadata and deserializes model files;
  it is not a sandbox for arbitrary Hub repositories or directories supplied by
  someone else. Only load artifacts you created or obtained from a source you
  trust, especially on a machine connected to robot hardware.
</Warning>
...
lerobot-teleoperate --robot.type=axol --teleop.type=axol_vr
...
| Kind | `type` | Class |
|---|---|---|
| Robot | `axol` | [`AxolRobot`](#axolrobot) |
| Teleoperator | `axol_vr` | [`AxolVRTeleop`](#axolvrteleop) |
| Camera | `zed` | [`ZedCamera`](#zedcamera) |

Teleoperation still uses the VR headset — open the [VR app](/operations/teleop) and connect to the robot machine. For the highest-rate data collection (out-of-process ZED capture, on-Jetson NVENC dataset encoding, headset video streaming), prefer the SDK's own [`collect-data`](/cli/collect-data) / [`run-policy`](/cli/run-policy), which are built on these same classes.

<Note>
  Requires the pinned LeRobot 0.6.1 release, Linux, and Python 3.12+ (the hosted installer bundles 3.13) on the robot machine; the `zed` camera additionally needs `pyzed` ([`zed.install`](/cli/zed-install)). See the [plugin README](https://github.com/almond-bot/axol/tree/main/plugins/lerobot_robot_axol) for adding it to an existing one-command-install `axol` environment.
</Note>

When installed from PyPI into the hosted Axol environment, the plugin's exact
version is preserved automatically across installer/control-panel Axol updates.
A direct, VCS, or otherwise customized plugin install instead blocks the hosted
force update before mutation; update that deployment manually with an explicit
plan for the same source.
...
| Method | Description |
|---|---|
| `get_observation()` | Returns fresh camera frames paired with the nearest sample from the Rust core's 240 Hz timestamped state history (+ torques if enabled) |
| `get_observation_with_capture_timestamp()` | Returns that synchronized observation plus its median sensor-exposure time on the `perf_counter` clock |
...
| `read_at_or_after(target_capture_perf_ts, timeout_ms=500)` | Block until a frame with `capture_perf_ts >= target` is available. Collection retains the returned exposure PTS and selects robot state on that same timeline. |

Policy observations do not sample joints before choosing images. `AxolRobot.get_observation()` never waits for an exposure after "now" (each camera pipeline adds its own latency, and waiting it out per camera is what ran the policy loop at a few Hz): it anchors on the newest exposure every camera has *already* delivered and takes each camera's retained frame nearest that anchor — the relay's shared-memory readers keep a short ring of exposures for this (`latest_capture_ts()` / `read_nearest(target, tolerance_s)`), an in-process reader contributes its newest frame. The same frame set is never served twice, so a loop faster than the cameras blocks for at most one frame period. The Rust realtime core retains a bounded 240 Hz feedback history; the frames' median sensor-exposure time selects the nearest state (in joint space, before optional FK). A silent camera (newest frame a couple of periods old), an unbracketed state, and excessive camera/state skew raise instead of silently producing a train/inference timing mismatch.
