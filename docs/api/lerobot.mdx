...
| `read_at_or_after(target_capture_perf_ts, timeout_ms=500)` | Block until a frame with `capture_perf_ts >= target` is available. Collection and policy inference retain the returned exposure PTS and select robot state on that same timeline. |

Policy observations do not sample joints before waiting for images. The Rust realtime core retains a bounded 240 Hz feedback history; after all fresh camera frames arrive, `AxolRobot.get_observation()` uses their median sensor-exposure time to select the nearest state (in joint space, before optional FK). Stale camera fallback, unbracketed state, and excessive camera/state skew raise instead of silently producing a train/inference timing mismatch.
