...
| `kd_host_max` | `float` (default `0.0`) | Ceiling on *total* host-side damping, including the spillover added when `kd` exceeds the motor firmware's range (legacy firmware). Host damping runs at ~100 Hz on a one-cycle-stale velocity, so it only stabilizes modes far below that rate: safe up to 40–45 on the shoulders (~2.3 Hz resonance) but destabilizing on joints that ring above ~10 Hz (elbow, wrists) — never raise it past hardware-verified values. `0` disallows any spillover beyond `kd_host` itself. |
