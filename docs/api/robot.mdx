...
| `experiments` | all off | `ControlExperiments`: opt-in tracking-accuracy experiments (steeper/slew-limited and load-dependent friction feed-forward, error-sign stiction compensation, a slow clamped position-error integrator, tracker-state wire velocity and inertia feed-forward, a stiction-breaking torque dither, and `wire_mode` — whether the MyActuator joints are commanded with the MIT impedance frame or one of the 0xA9 / 0x73 position closed-loop frames), each a `--axol.experiments.<name>` flag for hardware A/Bs. With the defaults the control law is exactly the production one. See the [teleop config flags](/operations/teleop-config) for the per-field table, including what the wire modes cost in telemetry. |
