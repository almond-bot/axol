...
| `kd` | `float` | Impedance velocity damping, encoded against `[0, 5]` on every motor family and firmware. (The MyActuator V4.4 changelog claims a widened 0–50 range, but hardware decodes the field against 0–5 on all versions; historical kd values tuned under the 0–50 encoding are 10× inflated — the loader migrates saved calibration values automatically.) Values above 5 are silently clamped, with the excess partially recoverable host-side up to `kd_host_max` (see below). |
