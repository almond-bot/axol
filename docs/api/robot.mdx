...
| `torque_limit` | `float` (default `inf`; `3.0` on `wrist_2` / `wrist_3`) | Cap (Nm) on the impedance *spring* torque a joint may develop against whatever blocks it. The realtime core enforces it every tick by never sending a position more than `torque_limit / kp` from the last measured one, so `kp · error` is bounded however far the operator's target runs ahead of a blocked joint (at the wrists' `kp=130`, 3 Nm is a ~1.3° window). Gravity feedforward is outside the cap — holding the arm's own weight is not pressing — and the friction / inertia / host-damping feedforwards are derived from the capped command, so they fall to zero while the joint is held. Set on the wrists because sustained pressing during teleop was overheating their small Damiao motors; `inf` leaves a joint uncapped. Override per joint: `--axol.left.wrist_2.torque_limit 2.0`. |
