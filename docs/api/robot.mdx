...
## Squeeze shaping (box mode)

`almond_axol.robot.squeeze` is the force side of [box mode](/guides/vr-interface#box-mode). An impedance arm pressing its gripper into a box exerts the wrench its joint springs make from the run-ahead of the command over the measured pose — a force at the gripper *mount*. The tool touches the box elsewhere (the parcel gripper at its folded blade's face beside the wrist and the fixed blade's tip 13 cm along), and how the force divides between those points is set by the moment about the mount, which a plain lateral run-ahead leaves to the arm's stiffness coupling: the face carries it, the tip lifts. `shape_squeeze(tau, kp, jac, rotation, normal, contacts, spring_caps, force_cap)` fits the run-ahead torque's contact part (a lateral force at each contact, `J^T [n; r × n]`, in the `1/kp` metric), replaces it by the same total split evenly over the contacts and saturated at the force cap — the tighter of `force_cap` and the force at which any spring-capped joint reaches its cap under the even split — and leaves the rest of the run-ahead (payload, lag, posture) alone. `Axol.set_squeeze(contacts, force_cap)` turns it on for both arms: every `motion_control` (`refresh_squeeze(left, right)`) then derives each arm's `SqueezeSpec` (the inward normal is the line between the two measured mount positions; `orient_contacts` mirrors the tool's points onto the side facing the box), asks both arms for the inward force their command would press with (`AxolArm.squeeze_estimate`) and hands them the *common* part — the mean of the two, floored at 0 — as the squeeze to cap (`shape_squeeze(..., shared=)`); what each arm's estimate differs from it by is the pair's carry and passes through in full, either sign. One arm's inward run-ahead is not a squeeze on its own: carrying the box sideways loads the trailing arm's normal exactly like a clamp and unloads the leading arm's by the same amount, so the mean is unchanged and the carry goes out on both — the leading arm pulls away from its box side by what the trailing one pushes, and the pair translates as one body under position control while the clamp between them is force-controlled. (Capping only the smaller estimate, an earlier version held the leading arm at the cap against its measured pose, so it could only be pushed along by the box and trailed the move.) `AxolArm` then applies it — `q_meas + tau_shaped / kp` — ahead of the spring-cap back-off. Realtime-core mode only; `Axol.squeeze_forces` reports the force each arm's last command applies.
