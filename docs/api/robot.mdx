...
contains its wheel, lift, heading-hold, and command-safety settings. Existing
integrations may continue importing `Cart` / `CartConfig`; those names are
compatibility aliases. Saved command configuration also retains the
`--cart.*` and `--cart_only` keys.
