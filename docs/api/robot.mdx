...
| Method | Description |
|---|---|
| `motion_control(left, right)` | Impedance (arm) + position-force (gripper: `[0, 1]` over the working opening — `PositionForceConfig.open_limit_deg`, 140° from closed for the parcel gripper's blade, or the whole stroke after `set_gripper_full_stroke(True)`, which box mode's angled grasp uses); primary control method. Python runs the model math (limits, gravity, pose-scheduled feedforward coefficients); the core renders the trajectory at 240 Hz |
