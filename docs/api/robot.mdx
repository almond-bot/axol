...
| Method | Units |
|---|---|
| `get_positions()` | rad (gripper: `[0, 1]` over the calibrated stroke, `1.0` = at the open stop) |
...
| Method | Description |
|---|---|
| `motion_control(left, right)` | Impedance (arm) + position-force (gripper: `[0, 1]` over the calibrated stroke, or over a shorter opening while `set_gripper_open_limit(deg)` is set — box mode's angled grasp holds the parcel gripper's blade at 140° that way); primary control method. Python runs the model math (limits, gravity, pose-scheduled feedforward coefficients); the core renders the trajectory at 240 Hz |
...
`detect_jelly` narrows a config to the Jelly hardware actually attached (the
wheel bus `can_alm_axol_b`, the lift bus) and returns `None` when there is
none — the same inference `axol teleop` and data collection use, so nothing
is ever enabled by hand.

```python
from almond_axol.robot import Jelly, JellyConfig, detect_jelly

jelly = Jelly()                              # jelly.* from the shared settings file
jelly = Jelly(JellyConfig(max_speed=5.0))    # explicit config overrides it

cfg = detect_jelly(JellyConfig())            # only the attached wheels / lift
if cfg is not None:
    jelly = Jelly(cfg)
```
...
The same model also serves the arm's kinematics on the control side: `GravityCompensator.mount_jacobian(arm_q, is_left=...)` returns the gripper mount's pose (the URDF gripper link — the frame the IK solver targets) and its `(6, 7)` geometric Jacobian at any joint configuration, from `mj_kinematics` alone.

## Squeeze lean (box mode)

The force side of [box mode](/guides/vr-interface#box-mode) lives in the IK worker, not the robot (`almond_axol.teleop.box.squeeze_lean`; see [`box_squeeze_lean`](/api/teleop#vrteleopconfig)). What the robot contributes is the model: `GravityCompensator.mount_jacobian` above, and `AxolArm.kp` — each arm's impedance stiffness per joint (Nm/rad, after the stiffness blend), which with the Jacobian gives the arm's Cartesian compliance at the mount, `J diag(1/kp) J^T`. The teleop core reads both arms' cached measured positions and `kp` before every box-mode frame (`almond_axol.teleop.core.measured_arms`) and forwards them to the worker, which offsets the gripper *targets* so the run-ahead the operator jogs becomes a pure force through the tool's contact points. Nothing on the robot's command path is touched: the arms are position controlled as ever, and the realtime core sees ordinary targets. (An earlier version reshaped every command on the robot from its measured pose; it worked on the bench and cost the pair its alignment on the robot, and is gone.)

One piece does live on the robot: the **blade hold** (`AxolArm._blade_hold`, bounded by `PositionForceConfig.hold_trim_deg`, 10° by default, `0` = off). While an opening limit is set (`set_gripper_open_limit`; box mode's angled grasp holds the parcel blade at 140°, short of its stop) the blade is held by the gripper motor's position loop alone, and a box clamped against its face folds it back by that loop's steady-state error under load — a give the arm's encoders never see. On every `motion_control` with a full-open command at a limit, the blade's measured shortfall from the limit (its own encoder) is integrated into the raw gripper command at 1°/s per degree — only once the blade is within 8° of the limit, so a limit change's travel doesn't wind it up — and decays over a second once the command leaves the limit. Sign-agnostic (it pushes whichever way the blade falls short), and the joint-limit clip still stops it at the stop. `AxolArm.blade_hold_deg` reads it; a `blade hold:` log line once a second reports the shortfall and shift while active.
