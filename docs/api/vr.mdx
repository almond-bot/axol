...
On a Jetson the relay detects GStreamer's `nvv4l2h264enc` and encodes with the **hardware H.264 encoder** (NVENC) instead of aiortc's software codec (`almond_axol.vr.hw_video`), pinning H.264 in the SDP offer. This keeps the stream real-time even at `HD1200`; machines without the element use the unchanged software path.
