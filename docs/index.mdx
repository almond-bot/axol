...
The Axol SDK runs the Almond Axol dual-arm robot. Every core operation — **teleoperation**, **gravity compensation**, **data collection**, **dataset replay**, **policy inference**, and **DAgger correction** — can be driven two ways: from the **web control panel** in a browser, or from the **`axol` CLI** in a terminal. Each operation page below shows both.
...
<CardGroup cols={2}>
  ...
  <Card title="DAgger Collection" icon="hand" href="/operations/dagger">
    Run a policy while correcting it from VR, recording the corrections.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="Hardware Overview" icon="microchip" href="/hardware">
    Axol, its mounting options, the Camera Kit, and the Compute Kit.
  </Card>
  <Card title="Hardware Setup" icon="screwdriver-wrench" href="/hardware-setup">
    Step-by-step setup for each mount (standalone, Owl Mount, Ox Cart, Jelly) and the cameras.
  </Card>
  ...
  <Card title="Mantis" icon="hand" href="/mantis/hardware">
    Handheld rigs for collecting demonstrations without moving the robot — hardware and tracking setup.
  </Card>
