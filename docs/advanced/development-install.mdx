...
This runs the [Quest-over-USB](/guides/quest-over-usb) adb setup, the [`zed.driver`](/cli/zed-driver) ZED Box (Duo / Mini) camera-driver upgrade, the board-IMU group access (`gyro.install`, the [powered cart](/guides/vr-interface#powered-cart)'s heading-hold yaw source), [`zed.install`](/cli/zed-install), [`gst.install`](/cli/gst-install), and [`gst.build-zed`](/cli/gst-build-zed) in order; each self-gates and no-ops off the target hardware (no ZED SDK / `apt` / factory ZED Box driver / board IMU). You can also run them individually.
