...
<Note>
  GPU-accelerated JAX isn't a dependency extra: the pip `jax[cudaNN]` wheels each bundle a fixed CUDA runtime, so a single pin would install the wrong CUDA major on a host with a different driver. Run [`axol jax.install`](/cli/provision) (also run by `axol provision`) to detect this device's CUDA version and install the matching `jax[cudaNN]` plugin. CPU is usually faster for the JAX IK solver used by `teleop`.
</Note>
...
This runs the [Quest-over-USB](/guides/quest-over-usb) adb setup, [`zed.install`](/cli/zed-install), [`gst.install`](/cli/gst-install), and [`gst.build-zed`](/cli/gst-build-zed) in order; each self-gates and no-ops if the ZED SDK or `apt` isn't present. You can also run them individually.
