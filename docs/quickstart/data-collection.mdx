...
- The `lerobot` extra installed and the GStreamer NVENC stack (for relaying the cameras to the headset). The [quick install](/installation#quick-install-recommended) sets up both; on a development install run `uv sync --extra lerobot` and [`axol gst.install`](/cli/gst-install) (see [Installation](/installation)).
...
<Note>
  The camera feeds are relayed to the headset during collection (requires the GStreamer NVENC stack, [`gst.install`](/cli/gst-install)), so you can line up takes from the robot's point of view.
</Note>
