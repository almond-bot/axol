...
- The `lerobot` and `video` extras installed. The [quick install](/installation#quick-install-recommended) includes both; on a development install run `uv sync --extra lerobot --extra video` (see [Installation](/installation)).
...
<Note>
  The camera feeds are relayed to the headset during collection (requires the `video` extra), so you can line up takes from the robot's point of view.
</Note>
