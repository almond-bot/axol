...
- **Axol set up** (if you are installing for an Axol): mounted, powered, and wired to the compute. See [Hardware Setup](/hardware-setup) for your mount.
- **Linux** with **systemd** (the installer registers a service).
- **(Optional) NVIDIA Jetson** — a ZED Box Orin NX 16GB, an AGX Orin (32GB / 64GB / Industrial), or a Thor T5000 — required for the GMSL-attached ZED cameras used in data collection and policy inference. The installer runs on any Linux host; the Jetson-specific tuning steps simply no-op elsewhere.
...
- On a factory-flashed **ZED Box** (Duo or Mini), upgrades an outdated GMSL camera driver to the release pinned for the ZED SDK ([`zed.driver`](/cli/zed-driver), also via [`axol provision`](/cli/provision)). The new driver only loads at boot, so when it upgrades, the installer **reboots the host** as its last step, which ends the SSH session. Reconnect in a minute or two. A no-op on any other machine.
- On a Jetson (Orin NX, AGX Orin, Thor), applies the real-time tuning ([`jetson.setup`](/cli/jetson-setup)) as the last [`provision`](/cli/provision) step — power mode, clocks, CAN interrupt, camera daemon — and the systemd unit re-applies it at every boot.
- Registers a root systemd service (`axol.service`) that runs [`axol serve`](/cli/serve) at boot and restarts it automatically if it ever goes down. The service is pointed at the installing user's home for dataset storage (`HF_LEROBOT_HOME=~<user>/.cache/huggingface/lerobot`), so [`collect-data`](/cli/collect-data) datasets land in that account rather than `/root` even though the service runs as root. The recording commands also hand each dataset back to that user after every episode save, so you never need `sudo` to read, copy, or upload your own recordings. The installer also puts a default ACL on that folder, so anything the root service creates there, calibration files included, stays read/write for that user. The service is written before provisioning runs, so it still starts if a provisioning step fails. The control panel then shows the failing step and can apply a fixed release through **Update**.
...
<Note>
  Both the hosted panel and direct CLI runs store datasets under `~/.cache/huggingface/lerobot` (or the panel's **Dataset root** setting), so recordings made from either are visible in both.
</Note>
