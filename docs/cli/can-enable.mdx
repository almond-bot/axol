...
The startup script also brings up [Jelly](/guides/vr-interface#jelly)'s pinned wheel bus (`can_alm_axol_b`) and the chest bus carrying the lift controller (`can_alm_axol_c`) when they were configured, and skips any interface — arm hub, Jelly, or chest — that isn't attached, so every hardware combination (Jelly-only, arm-only, all of them) comes up cleanly. You rarely need to run this for Jelly or its lift anyway: [`axol teleop`](/cli/teleop) brings their interfaces up itself when it starts.
