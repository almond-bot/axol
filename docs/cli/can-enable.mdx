...
The startup script also brings up [Jelly](/guides/vr-interface#jelly)'s pinned wheel bus (`can_alm_axol_b`) and, when the lift controller has its own adapter, the chest bus (`can_alm_axol_c`); a lift sharing the wheel bus rides `can_alm_axol_b` and needs no extra interface. It skips any interface — arm hub, Jelly, or chest — that isn't attached, so every hardware combination (Jelly-only, arm-only, all of them) comes up cleanly. You rarely need to run this for Jelly or its lift anyway: [`axol teleop`](/cli/teleop) brings their interfaces up itself when it starts.
