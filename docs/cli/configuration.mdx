...
<CardGroup cols={3}>
  ...
  <Card title="Control panel" icon="sliders" href="/guides/control-panel#settings">
    Save a value once on the robot host and it's applied to every operation launched from the panel.
  </Card>
</CardGroup>
...
Precedence is **defaults → `--config_path` file → CLI flags** (later wins), so flags always override file values, which override the built-in defaults. Operations launched from the [control panel](/guides/control-panel#settings) slot the host's saved settings in between the defaults and the file.
