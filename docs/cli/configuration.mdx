...
<Note>
  On a calibrated robot the "defaults" already include that machine's own measurements: per-joint gains, friction parameters, and link centres of mass saved by [`tune.pid --save`](/cli/tune-pid) / [`tune.friction --save`](/cli/tune-friction) / [`tune.gravity --save`](/cli/tune-gravity) to `~/.almond/calibration.json` are loaded into `AxolConfig` at construction, replacing the shared constants. Config files and CLI flags still override them like any other default.
</Note>
