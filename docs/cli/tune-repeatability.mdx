...
| `--no-gripper` | Run on the gripperless SKU — the gripper motor is never enabled or calibrated (`--gripper-torque-limit` is then ignored) |
...
| `--stiffness FLOAT` | Arm stiffness scale 0–1 — lower softens the motors and reduces tracking jitter (default: 1.0, the tuned production gains) |
