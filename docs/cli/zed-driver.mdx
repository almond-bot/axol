...
The download is pinned to the exact reviewed SHA-256 and package/version/architecture metadata and is staged through an exclusive temporary file before installation. Stereolabs does not publish an independent signature or checksum for this artifact, so the committed digest protects against later server changes but is not independent proof of the original artifact's provenance. Any changed vendor artifact is rejected until its bytes are reviewed and the code pin is deliberately updated.
