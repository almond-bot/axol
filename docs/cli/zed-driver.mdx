...
<Warning>
  The new driver is a kernel module and only loads at boot, so a **reboot is required** after the upgrade. Run standalone, the command only prints a notice. [`axol provision`](/cli/provision#reboots), the installer, and the `axol serve` self-updater reboot for it at their own safe point (the self-updater only while no operation is running).
</Warning>
