...
`--joints` selects which joints are *swept*; every other joint holds home. The whole arm is still brought up and held (the realtime core enables every motor on the bus, so a partially populated bench arm cannot run this test), and a subset run skips the grasp step — if the gripper is among the selected joints it is cycled open↔close holding nothing.
