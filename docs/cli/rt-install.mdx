...
recording I/O pressure can never fault a CAN thread mid-tick. The step escalates on
its own: directly as root (the installer and the system service), otherwise
through passwordless or already-cached `sudo`, or one password prompt on a
terminal. It is skipped entirely when the binary already carries both
capabilities, which reading needs no privilege for. Only a run with none of
those fails, and it prints the exact `sudo setcap` command to run once. The control core refuses to arm if its requested real-time
