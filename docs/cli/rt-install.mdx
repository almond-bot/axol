...
The package and the binary must speak the same wire protocol. Python declares its generation in the core's config (`proto 2`), and a binary built from a different layout refuses to configure — before any motor is touched — with a message naming the fix (`config: client speaks wire protocol …, this core speaks …`); the session then fails with `axol-rt exited while being configured`. That is the signal to rebuild: `axol rt.install`, or `cargo build --release` in `rust/axol-rt` for a checkout whose `target/release/` binary has gone stale.
