...
The matching wheel is downloaded over HTTPS through an exclusive temporary file and is accepted only when its distribution name, SDK version, Python/architecture tag, dependency names, native extension, and archive paths are valid. Stereolabs does not publish per-wheel signatures or checksums, so HTTPS remains the provenance boundary for these SDK-version-specific wheels; validation fails closed on unexpected metadata rather than installing it.
