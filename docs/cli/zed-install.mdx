...
<Note>
  Requires the **ZED SDK** to be installed at `/usr/local/zed` first — the command reads the SDK version from its headers to pick the right wheel, and exits with an error pointing to the [Stereolabs download page](https://www.stereolabs.com/developers/release) when it's missing. The [quick install](/installation#quick-install-recommended) runs this automatically whenever the SDK is present.
</Note>
