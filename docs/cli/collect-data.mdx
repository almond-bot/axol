...
During collection the camera feeds are also relayed to the headset over WebRTC (hardware-encoded on the Jetson's NVENC, installed by [`gst.install`](/cli/gst-install)), so the operator sees the overhead and wrist views while recording — switched with the right thumbstick, same as teleop. Collection uses the same GPU-resident `gst_zed` pipeline teleop does (falling back to the SDK grab + in-Python NVENC when the zed-gstreamer plugins aren't built — see [`gst.build-zed`](/cli/gst-build-zed)), but a **single grab per camera** serves both the dataset (raw frames, timestamped from the buffer PTS to align with joint samples) and the headset (encoded H.264) — no second capture. See the [VR Interface guide](/guides/vr-interface#camera-views).
