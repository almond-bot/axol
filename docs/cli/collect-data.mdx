...
Records teleoperation episodes using VR controller inputs and locally attached ZED cameras. Saves to a [LeRobot](https://github.com/huggingface/lerobot)-format dataset. Loops until `Ctrl+C`. On a clean stop the arms are parked — eased down to the all-zeros pose with the grippers holding their current value — before the motors release; see `--park_on_exit`.
...
| `--park_on_exit false` | Opt out of parking the arms on a clean stop. By default (`true`) the arms ease down to the all-zeros pose (a collision-aware move planned by the IK worker) before the motors release, with the grippers holding their current value. The park is skipped after an error (a possibly-faulted robot is never commanded), capped at ~30 s, and a second `Ctrl+C` aborts it and releases the motors immediately. |
