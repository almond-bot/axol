...
During collection the camera feeds are also relayed to the headset over WebRTC (requires the GStreamer WebRTC stack, [`gst.install`](/cli/gst-install)), so the operator sees the overhead and wrist views while recording — switched with the right thumbstick, same as teleop. This is the same SDK-grab → NVENC → `webrtcbin` path teleop uses. See the [VR Interface guide](/guides/vr-interface#camera-views).
