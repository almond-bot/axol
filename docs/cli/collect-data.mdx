...
During collection the camera feeds are also relayed to the headset over WebRTC (requires the zed-gstreamer + NVENC stack, [`gst.install`](/cli/gst-install)), so the operator sees the overhead and wrist views while recording — switched with the right thumbstick, same as teleop. Collection uses the same GPU-resident `gst_zed` pipeline teleop does, but a **single grab per camera** serves both the dataset (raw frames, timestamped from the buffer PTS to align with joint samples) and the headset (encoded H.264) — no second capture. See the [VR Interface guide](/guides/vr-interface#camera-views).
