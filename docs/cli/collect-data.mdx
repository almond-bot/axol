...
| ↳ `stereo: true` on `overhead` | The overhead is a stereo ZED X. The dataset then records two observation keys, `overhead_left` and `overhead_right`, backed by a single grab. |
