...
<Warning>
  `axol serve` is a trusted-LAN control service, not a multi-user authenticated API. Keep ports 8000/8001 behind the robot network or a host firewall; do not expose them directly to the internet. Browser requests are limited to the hosted Axol UI, loopback development, and the exact LAN IP/local hostnames registered when `axol serve` starts; the server never trusts a request's `Host` header to grant access. Native clients on the LAN intentionally remain compatible without an `Origin` header. A custom hosted UI or DNS alias must be explicitly added with the comma-separated `AXOL_ALLOWED_BROWSER_ORIGINS` environment variable.
</Warning>
