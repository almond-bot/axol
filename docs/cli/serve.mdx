...
<Note>
  The operations launched from the panel share the CLI's Python entry points — [`teleop`](/cli/teleop), [`gravity-comp`](/cli/gravity-comp), [`waypoints`](/cli/waypoints), [`collect-data`](/cli/collect-data), [`replay-dataset`](/cli/replay-dataset), and [`run-policy`](/cli/run-policy). Teleop and gravity-comp run as their **own subprocess** (their 240–250 Hz control loops must not share the serve interpreter — GIL holds from the web handlers stretch control ticks into felt roughness); the rest run in-process inside `serve`. Anything those commands need on the host (CAN, the `lerobot`/`sim` extras, ZED access) is required here too.
</Note>
