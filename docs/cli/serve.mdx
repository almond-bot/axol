...
sudo "$(command -v axol)" serve              # hardware development (CAN setup, ZED daemon)
```

The root development form lets the panel run CAN discovery/setup and restart
the ZED daemon. Datasets go to `$HF_LEROBOT_HOME` (or the panel's **Dataset
root** setting); when that lives in a user's home, each recording is handed
back to that user after every episode save, as with the installed service.
