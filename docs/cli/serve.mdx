...
<Tip>
  For a guided walkthrough of the control panel — connecting the workstation, Axol, and ZED box, and running each operation — see the [Web Control Panel guide](/guides/control-panel).
</Tip>
...
The server looks for the built web bundle at `web/app/dist`. If it isn't built, `axol serve` still runs and the **API is available**, but there's no UI to open. Build it once with:
