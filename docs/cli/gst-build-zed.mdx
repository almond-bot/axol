...
<Note>
  Requires the **ZED SDK** at `/usr/local/zed` and the Jetson build toolchain. Idempotent: its stamp records the pinned ref, patch digest, and SHA-256 + resolved path of both source-plugin binaries proven to come from that build's CMake install manifest. Collection and policy re-hash the exact binary GStreamer selects, so a later stock upgrade or `GST_PLUGIN_PATH` override safely falls back to the SDK until this command is rerun. The build is best-effort on machines without the SDK/toolchain. Installing into the system GStreamer plugin dir needs root: run as root, or you'll be prompted for `sudo` once.
</Note>
