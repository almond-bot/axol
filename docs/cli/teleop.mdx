...
With `--cameras` set, teleop opens the locally attached ZED cameras by serial number and relays them to the headset over WebRTC, so the operator sees the robot's cameras instead of (or alongside) passthrough. The right thumbstick switches views — see the [VR Interface guide](/guides/vr-interface#camera-views) for the view system and controls. Requires `pyzed` ([`zed.install`](/cli/zed-install)) and the GStreamer WebRTC stack ([`gst.install`](/cli/gst-install) — the hosted installer runs it).

Teleop and [`collect-data`](/cli/collect-data) share one video path: frames are grabbed with the **ZED Python SDK** (`ZedCamera` → RGB numpy) and pushed into a GStreamer pipeline that converts and encodes on the Jetson's **NVENC** hardware encoder (`nvvidconv` → `nvv4l2h264enc`), then **WebRTC is sent entirely from GStreamer** via `webrtcbin`. No encoded frames return to Python — RTP/SRTP/ICE all run in C — so the control loops never pay for video traffic. Each frame is encoded the instant it's captured (teleop asks the cameras for 60 fps capture, falling back to the camera default when unsupported), so there's no fixed-rate sampling staleness. A **stereo overhead** (auto-detected from the overhead serial) grabs both eyes and is relayed as two sources, `overhead_left` / `overhead_right`, for per-lens rendering.
...
For low-latency encode the NVENC/VIC engine clocks must be pinned to their maximum (the default Jetson devfreq governor scales them for throughput, not latency, which roughly triples per-frame encode time). This is handled by [`axol jetson.setup`](/cli/jetson-setup), run at boot by the installer's systemd unit — not at teleop startup. If no GStreamer hardware encoder is present at all, `webrtcbin` falls back to software H.264 — `SVGA` is then the recommended resolution.
