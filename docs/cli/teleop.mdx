...
| `--umi` | Handheld UMI rig bench mode: the Quest triggers drive the two real grippers on `can_alm_umi_l/r` (see [`can.setup --umi`](/cli/can-setup)) while the arms exist only as the headset's translucent URDF overlay — no robot, no cameras, no recording. Forces the same absolute-mapping profile as `collect-data --umi`, so this validates controller↔URDF alignment, trigger→gripper actuation, and IK tracking before collecting data. Mutually exclusive with `--sim`. |
...
Teleop and [`collect-data`](/cli/collect-data) share one **GPU-resident** video path (`almond_axol.video.gst_zed`): the zed-gstreamer source elements (`zedxonesrc` mono / `zedsrc` stereo) grab and **NVENC**-encode (`nvvidconv` → `nvv4l2h264enc`) entirely on the GPU, and Python only ever sees the encoded H.264 access units — which **aiortc** forwards as pre-encoded packets (RTP/SRTP/ICE + SDP signaling), with no Python encode step. Teleop asks the cameras for 60 fps capture (falling back to the camera default when unsupported). Stereo ZED X cameras are auto-detected from their serial: a **stereo overhead** grabs both eyes on one pass and is relayed as two sources, `overhead_left` / `overhead_right`, for per-lens rendering, while a **stereo wrist** is relayed as a single source under its slot name (one feed, like a mono camera). When the gst stack is unavailable the path falls back to the ZED SDK grab (`ZedCamera`) plus in-Python NVENC (`almond_axol.video.hw_video`).
