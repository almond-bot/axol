...
| `--teleop.record PREFIX` | Capture the whole teleop pipeline (raw VR pose through measured joints) to `PREFIX_{ik,cmd,meas}.npz` for offline jitter attribution with [`diag.teleop-jitter`](/cli/diag-teleop-jitter). Measured joints and torques come from the Rust core feedback stream at 240 Hz. Captures the latest engage→disengage segment: files are written at disengage, re-engaging starts over. Costs one buffer copy per tick; unset (the default) records nothing. |
