...
| `--mantis` | Mantis bench mode: the Quest triggers drive the two real grippers on `can_mantis_l/r` (auto-detected by [`can.setup`](/cli/can-setup)) while the arms exist only as the headset's translucent URDF overlay — no robot, no cameras, no recording. Forces the same absolute-mapping teleop profile **and tight-tracking IK solver profile** as `collect-data --mantis` (higher pose weights, no manipulability bias, 2 cm collision margin — explicit `--kinematics.<field>` flags still win), so this validates controller↔URDF alignment, trigger→gripper actuation, and IK tracking before collecting data. Mutually exclusive with `--sim`. |
