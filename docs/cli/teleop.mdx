...
| `--rt` | **Experimental.** Run the CAN control loop in the `axol-rt` Rust realtime core: Python keeps VR, IK, and the model feedforwards (gravity, friction, inertia), and ships per-joint impedance and gripper commands to the core, which owns the buses in both directions, paces a hard 240 Hz loop interpolating between Python's ~120 Hz targets, computes the host-damping torque in-core each tick from same-tick feedback (the pose-scheduled coefficients ride each command), and streams measured positions/velocities/torques back to Python as per-tick telemetry packets (Python's own CAN receive path is muted while armed). Guarded return and the contact watchdogs work as in classic mode. Requires the `axol-rt` binary (`cargo build --release` in `rust/axol-rt`, or set `AXOL_RT_BIN`). Hardware only. See `rust/axol-rt/README.md`. |
