...
| `--mantis_source {quest,lighthouse,ultimate}` | Mantis pose source shown in the control panel (default: `quest`). Quest connects directly over WebXR; Lighthouse maps to the `survive` tracker backend; Ultimate maps to `ultimate`. Direct CLI runs still launch `tracker.bridge` separately for either tracker choice. |
