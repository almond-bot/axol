...
On a Jetson, the relay automatically encodes with the **hardware H.264 encoder** (NVENC via GStreamer's `nvv4l2h264enc`) instead of aiortc's software codec, keeping latency low even at `HD1200`. If the GStreamer element isn't installed, the relay falls back to software encoding — `SVGA` is then the recommended resolution.
