...
| `--rt` | **Experimental.** Run the CAN control loop in the `axol-rt` Rust realtime core: Python keeps VR, IK, and the model feedforwards (gravity, friction, inertia), and ships per-joint impedance and gripper commands to the core, which owns the buses, paces a hard 240 Hz loop interpolating between Python's ~120 Hz targets, and computes the host-damping torque in-core each tick from same-tick feedback (the pose-scheduled coefficients ride each command). Guarded return and the contact watchdogs work as in classic mode. Requires the `axol-rt` binary (`cargo build --release` in `rust/axol-rt`, or set `AXOL_RT_BIN`). Hardware only. See `rust/axol-rt/README.md`. |
