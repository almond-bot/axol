...
| `--rt` | **Experimental.** Run the CAN control loop in the `axol-rt` Rust realtime core: Python keeps VR, IK, and the model feedforwards (gravity, friction, inertia), and ships per-joint impedance and gripper commands to the core, which owns the buses in both directions and paces a hard 240 Hz loop: a velocity/acceleration-limited tracker (the same trapezoid as the Python teleop filter, golden-ported) renders the trajectory toward Python's ~120 Hz targets, and the velocity, friction, inertia, and host-damping torques are all computed in-core each tick from the tracker and same-tick feedback (the pose-scheduled gains ride each command). Measured positions/velocities/torques stream back to Python as per-tick telemetry packets (Python's own CAN receive path is muted while armed). Guarded return and the contact watchdogs work as in classic mode. Requires the `axol-rt` binary — installed by [`provision`](/cli/provision) / [`rt.install`](/cli/rt-install) (or `cargo build --release` in `rust/axol-rt`, or set `AXOL_RT_BIN`). Hardware only. See `rust/axol-rt/README.md`. |
