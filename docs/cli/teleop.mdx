...
| `--rt` | **Experimental.** Run the CAN control loop in the `axol-rt` Rust realtime core: Python keeps VR, IK, and the slow model math, and ships per-joint impedance and gripper commands to the core, which owns the buses in both directions and paces a hard 240 Hz loop. A velocity/acceleration-limited tracker (the same trapezoid as the Python teleop filter, golden-ported) renders the trajectory toward Python's ~120 Hz targets; the classic low-pass command derivatives compute smooth friction/inertia feedforwards from that trajectory, and host damping uses the latest feedback within one core tick (the pose-scheduled gains ride each command). Measured positions/velocities/torques stream back to Python as per-tick telemetry packets (Python's own CAN receive path is muted while armed). Guarded return and the contact watchdogs work as in classic mode. Requires the `axol-rt` binary — installed by [`provision`](/cli/provision) / [`rt.install`](/cli/rt-install) (or `cargo build --release` in `rust/axol-rt`, or set `AXOL_RT_BIN`). Hardware only. See `rust/axol-rt/README.md`. |
...
| `--teleop.record PREFIX` | Capture the whole teleop pipeline (raw VR pose through measured joints) to `PREFIX_{ik,cmd,meas}.npz` for offline jitter attribution with [`diag.teleop-jitter`](/cli/diag-teleop-jitter). Rust mode records measured joints and torques from the core feedback stream at 240 Hz; Python mode uses its configured control rate. Captures the latest engage→disengage segment: files are written at disengage, re-engaging starts over. Costs one buffer copy per tick; unset (the default) records nothing. |
