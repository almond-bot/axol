...
| `--rt` | **Experimental.** Run the CAN control loop in the `axol-rt` Rust realtime core: Python keeps VR, IK, and the full `motion_control` math, but ships the per-joint impedance commands to the core, which owns the buses and paces a hard 240 Hz loop, interpolating between Python's ~120 Hz targets. Requires the `axol-rt` binary (`cargo build --release` in `rust/axol-rt`, or set `AXOL_RT_BIN`). Hardware only; the gripper is not driven yet. See `rust/axol-rt/README.md`. |
