...
| `--cart.imu BOOL` | Feed the carrier board's IMU into the cart's straight-line heading hold as its yaw reference (independent of the cameras). Without it the hold is inert and the heading wanders several degrees per stroke; a missing gyro only logs a warning. Default: `true`. |
| `--cart.accel R`, `--cart.decel R`, `--cart.jerk J` | Command ramp shape, in full-stick units per second (per second² for jerk). `accel` applies while the command grows, `decel` while it shrinks toward zero, so stops are brisker than launches; `jerk` rounds both into an S-curve (0 for a plain trapezoid). Defaults: `0.5`, `1.0`, `2.0`. |
| `--cart.traction BOOL` | Traction guard: eases the command ramp while a wheel has lost the floor, judged from the wheel motors' torque feedback (a loaded wheel carries its share of the push, one in the air shows only its own inertia). A tall base with its mass off the wheelbase centre lifts a wheel under the weight shift of a launch or stop and veers along the remaining pair's diagonal — no wheel command can push through a wheel that isn't touching, so the guard asks for less acceleration only while that is the case and the ramp stays brisk otherwise. `--cart.traction_light`, `--cart.traction_floor`, `--cart.traction_torque_min` tune it; `--cart.traction_log true` traces it per stroke. Default: `true`. |
| `--cart.wheel_scale [FL,FR,BL,BR]` | Per-wheel command multipliers compensating effective-radius differences between the omni wheels, which otherwise slide the base sideways even with the heading held. Measured automatically by [`axol diag.base-calibrate`](/cli/diag-base-calibrate) (the cart drives six short strokes tracked by the overhead ZED), or from two tape-measured strokes with `almond_axol.robot.cart.solve_wheel_scale`. Default: `[1,1,1,1]`. |
| `--cart.<field> VALUE` | Any other cart tunable — peak wheel speed (`--cart.max_speed`), turn rate, stick deadzone, park-hold stiffness, … The full set is the `CartConfig` dataclass, and each field also appears under **Advanced → Cart** in the [control panel](/guides/control-panel#settings). |
