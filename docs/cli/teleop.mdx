...
| `--teleop.box_mode BOOL` | Start in [box mode](/guides/vr-interface#box-mode) (default: `false`): the grippers face each other like two hands on a box and **one grip drives both arms as a rigid pair** (the other grip hands over the lead); thumbsticks jog the pair instead of driving Jelly. Can also be toggled live from the headset's **Box** HUD button, by clicking both thumbsticks together, or from the control panel's **Session settings** card. The pair's tunables are `--teleop.box_jog_speed` (m/s, default `0.15`), `--teleop.box_jog_yaw_speed` (rad/s, `0.6`), `--teleop.box_width_speed` (m/s, `0.08`), `--teleop.box_width_min` / `box_width_max` (m, `0.10` / `0.70`) and `--teleop.box_align_duration` (s, `1.5` — how long the engage-time blend into the parallel pair takes). |
