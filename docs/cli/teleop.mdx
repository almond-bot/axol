...
| `--mantis` | Mantis bench mode: the two real grippers run on `can_mantis_l/r` (auto-detected by [`can.setup`](/cli/can-setup)) while the arms exist only as the VR client's translucent URDF overlay — no robot, cameras, or recording. The grippers enable immediately for teleop (unlike collection, which enables them only during a take). Forces the same absolute-mapping and tight-tracking IK profiles as `collect-data --mantis`. In the control panel, `mantis_source` selects Quest (direct WebXR), Lighthouse, or Ultimate; tracker choices start their saved bindings and trigger readers automatically. Direct CLI use still runs [`tracker.bridge`](/cli/tracker) separately. Mutually exclusive with `--sim`. |
| `--mantis_source {quest,lighthouse,ultimate}` | Mantis pose source (default: `lighthouse`), configured in the control panel under **Settings → Mantis**. Quest connects directly over WebXR; Lighthouse maps to the `survive` tracker backend; Ultimate maps to `ultimate`. Direct CLI runs still launch `tracker.bridge` separately for either tracker choice. |
...
axol teleop --sim --teleop.absolute_mode true     # Mantis alignment check (URDF overlay in headset)
