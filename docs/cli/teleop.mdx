...
| `--teleop.jitter_record PREFIX` | Capture the whole teleop pipeline (raw VR pose through measured joints) to `PREFIX_{ik,cmd,meas}.npz` for offline jitter attribution with [`diag.teleop-jitter`](/cli/diag-teleop-jitter). Costs one buffer copy per tick; unset (the default) records nothing. |
