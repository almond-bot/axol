...
| `--cart_only` | Drive **only** [Jelly](#powered-cart) — the arms are never constructed and the Axol hub CAN channels are never opened, so no arm hardware need be connected. Jelly is implied (`--cart.enabled` isn't consulted). Mutually exclusive with `--sim`. |
...
| `--cart.enabled BOOL` | This robot has [Jelly](#powered-cart) (x-drive base + telescoping lift). When set, the headset thumbsticks drive the base/lift (operator mobility only — never recorded, never policy-controlled). Default: `false`. |
| `--cart.channel <ch\|null>` | SocketCAN interface for Jelly's wheel motors; `null` disables the wheels (lift-only). Default: `can_alm_axol_b`, pinned by [`can.setup`](/cli/can-setup). |
| `--cart.lift BOOL` | The telescoping lift is present (the jelly_legs controller on the chest bus). The chest bus being down only disables the lift with a warning — Jelly still drives. Default: `true`. |
...
| `--cart.imu BOOL` | Feed the carrier board's IMU into Jelly's straight-line heading hold as its yaw reference (independent of the cameras). Default: `false`. |
| `--cart.<field> VALUE` | Any other Jelly tunable — peak wheel speed (`--cart.max_speed`), turn rate, command slew, stick deadzone, park-hold stiffness, … The full set is the `JellyConfig` (`CartConfig` compatibility alias) dataclass, and each field also appears under **Advanced → Jelly** in the [control panel](/guides/control-panel#settings). |
...
axol teleop --cart.enabled true                   # drive Jelly from the thumbsticks
axol teleop --cart_only                            # drive only Jelly (no arms, no hub)
...
Robots fitted with **Jelly** (x-drive omni base + telescoping lift) let the operator drive the whole robot from the headset thumbsticks — left stick translates, right stick rotates, the stick clicks run the lift. Enable it with `--cart.enabled true` (or the control panel's **Jelly** toggle); Jelly parameters retain the nested `cart` config key (`--cart.max_speed`, `--cart.channel`, `--cart.imu`, …) for compatibility. Jelly motion is **operator mobility only** — it's never recorded into a dataset and a policy never drives it.

`--cart_only` runs *just* Jelly: the arms are never constructed and the Axol hub CAN bus is never opened, so you can drive Jelly without the arm hardware connected. Jelly brings its own CAN interface up on start ([`can.setup`](/cli/can-setup) pins it as `can_alm_axol_b`), so no separate [`can.enable`](/cli/can-enable) step is needed. For the full control mapping see the [VR interface guide](/guides/vr-interface#powered-cart).
