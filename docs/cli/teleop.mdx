...
| `--mantis_source {quest,lighthouse,ultimate}` | Mantis pose source. A direct `--mantis` run inherits the saved **Mantis settings → Tracking** choice (and its logical channel map; for Quest, its calibration key); without saved state the source defaults to `lighthouse`. This flag or a config-file value overrides the saved choice. Quest waits for the WebXR client. Lighthouse (`survive`) and Ultimate (`ultimate`) start and stop their saved tracker binding and CAN trigger readers automatically in direct CLI as well as control-panel runs; do not launch a second `tracker.bridge`. Managed tracker startup waits for two fresh poses and two live trigger nodes, then requires both triggers squeezed together and released to engage at the rigs' current pose. After tracking loss or Reset, repeat that both-squeeze→release. |
| `--mantis_grippers_only` | Run Mantis teleop without tracking: the rig triggers drive the two grippers over CAN and nothing else starts — no VR server, tracker bridge, cameras, or transforms. Switched on automatically when the selected Lighthouse/Ultimate source is not set up (runtime missing, no dongle, no saved binding …), so a Mantis teleop only ever needs CAN; set it explicitly to skip tracking even when it is set up. Both triggers must read live and released before the grippers enable (each opens fully to find its hard stop); a stale trigger stream stops the run. Quest never falls back — it is checked live by the WebXR handshake. |
...
axol teleop --mantis --mantis_grippers_only true  # triggers → grippers only, no tracking
