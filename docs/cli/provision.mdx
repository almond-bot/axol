...
- [`rt.install`](/cli/rt-install) — the **`axol-rt` realtime core** binary for [`--rt` mode](/cli/teleop) (Rust toolchain via rustup if needed; sources fetched at the installed package's exact ref on tool installs). Self-gates on network/toolchain availability.
