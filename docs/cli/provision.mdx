...
Provisioning holds the host-wide update lock, which is root-only. When run from a terminal as a regular user, the command re-runs itself under `sudo` with the same Python interpreter (one password prompt); without a usable `sudo` it exits with the exact command to run instead of a traceback.
