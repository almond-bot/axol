...
- **board IMU** (`gyro.install`) — grants the `imu` group access to the carrier board's BMI088 sampling timer, [Jelly](/guides/vr-interface#powered-cart)'s heading-hold yaw reference, so teleop can start it without root. Self-gates on the board driver's presence.
