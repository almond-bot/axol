...
| Flag | Description |
|---|---|
| `prefix` | Flight-recorder prefix from `axol teleop --teleop.jitter_record PREFIX` (reads `<prefix>_cmd.npz`, and `<prefix>_ik.npz` for the engaged-span clip). A bare name (no `/`) resolves in the recordings directory `~/.almond/recordings/` — where bare `--teleop.jitter_record` names are written. **Optional**: omitted, the newest recording in that directory is used |
