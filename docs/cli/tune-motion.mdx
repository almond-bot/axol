...
| `--rt` | **Experimental.** Drive the wire through the `axol-rt` Rust realtime core: the core owns CAN at 240 Hz (in-core trapezoid tracking, friction/inertia feedforward, velocity damping) while playback streams targets — same switch as [`teleop --rt`](/cli/teleop). Stored on the run artifact (`rt`), so classic and rt runs can be A/B-compared. Requires the `axol-rt` binary |
