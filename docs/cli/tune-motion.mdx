...
| `--loop-hz HZ` | Realtime-core tick rate override for A/B runs (default follows `--controller`: 240 impedance, 400 position) — e.g. `--controller position --loop-hz 240` to separate the rate from the controller |
