...
For the handheld UMI data-collection rig (a second adapter of the same model, each channel wired to one Damiao gripper), pass `--umi`. It uses its own udev rule file and startup script, so the robot and the rig can be configured on the same machine:

- Left gripper → `can_alm_umi_l`
- Right gripper → `can_alm_umi_r`
