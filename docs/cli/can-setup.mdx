...
For the Mantis UMI handheld data-collection rig, pass `--umi`. The rig uses the **same dual-channel CAN board as the arm hub** (one USB device pinned by serial), with one gripper bus per channel — each bus carries that side's Damiao gripper motor (ID `0x008`) and the Mantis UMI trigger node (ID `0x009`):
...
The Mantis UMI profile gets its own udev rule file, startup script, and hotplug bring-up unit, so the robot and the rig can be configured on the same machine, and a replug of the handheld (common — it's handheld) brings its interfaces back up without operator action. Serials claimed by one profile's rules are excluded when scanning for the other's adapter, so having both plugged in stays unambiguous.
