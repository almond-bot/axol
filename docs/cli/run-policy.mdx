...
**Stopping parks the arms.** Ending a run — `Ctrl+C`, `q`, or **Stop** in the control panel — plays that same return to rest and only then torques the motors off, so raised arms no longer drop under gravity on the way out. The teardown move is capped at 3 s so it always finishes inside the panel's stop window; if it runs out of time or hits something, the arms are torqued off where they are. It is skipped when there is nothing to park or no safe way to park it: when the arms are already at rest, when a limp gravity-comp hold has left them in the operator's hands, when the IK worker that plans the path never came up, when the CAN bus has stopped reporting positions, or when an episode worker has not proved it left the robot. A second `Ctrl+C` cannot interrupt it.
