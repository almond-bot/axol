...
| `--robot_config.rt true` | **Experimental.** Drive the wire through the `axol-rt` Rust realtime core: the core owns CAN at 240 Hz (in-core trapezoid tracking, friction/inertia feedforward, velocity damping) while the policy's action stream is upsampled by the core's tracker — same switch as [`teleop --rt`](/cli/teleop). Requires the `axol-rt` binary. |
