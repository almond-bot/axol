...
| ↳ `stereo: true` on `overhead` | The overhead is a stereo ZED X. The policy then receives the `overhead_left` / `overhead_right` observation keys — match the dataset the policy was trained on. (On the [control panel](/guides/control-panel#cameras) this is auto-detected from the serial; on the CLI you set it.) |
