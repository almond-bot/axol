...
| ↳ `stereo: true` on `overhead` | The overhead is a stereo ZED X. The policy then receives the `overhead_left` / `overhead_right` observation keys — match the dataset the policy was trained on. |
