...
| `--device STR` | PyTorch device for policy inference (default: `cuda`). The hosted aarch64/Jetson dependency set has CPU-only PyPI Torch 2.10; use `cpu`, remote inference, or an explicitly managed JetPack-compatible Torch + Torchvision build |
