...
<Note>
  The hosted service only replays datasets below its root-owned store at
  `/var/lib/almond-axol/datasets`; arbitrary filesystem paths remain available
  to direct non-root CLI use.
</Note>
