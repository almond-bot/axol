...
No cameras, teleop, or policy server are involved: replay only drives the arms, so the robot config carries no cameras. The robot is first moved to the rest pose (the same collision-aware return-to-rest [`run-policy`](/cli/run-policy) uses) so the arm starts where every recorded episode does — episodes are recorded from rest, so the first replayed action is ~rest and there's no jump. Each frame is sent at the dataset's recorded fps to reproduce the original timing, and on exit the arms are parked — eased to the rest pose and then down to the all-zeros pose — before the motors release.

Pass `--loop` to replay the episode continuously (returning to rest between takes) until you stop it with `Ctrl+C` (or **Stop** in the [control panel](/guides/control-panel)); either way a clean stop parks the arms before the operation exits (see `--park_on_exit`).
...
| `--park_on_exit false` | Opt out of parking the arms on a clean stop. By default (`true`) the arms ease to the rest pose and then to the all-zeros pose before the motors release. The park is skipped after an error (a possibly-faulted robot is never commanded), capped at ~30 s, and a second `Ctrl+C` aborts it and releases the motors immediately. |
| `--rest_pose_left LIST` | Rest pose for the left arm — targeted by the pre-replay/between-take resets and the park's first leg — as 7 joint angles in radians (`Joint` enum order), one inline YAML/JSON list: `--rest_pose_left "[-0.157, 0, 0, 0.314, 0, 0, -0.157]"`. Default `null` keeps the built-in rest pose. Use `rest_pose_right` for the right arm. |
