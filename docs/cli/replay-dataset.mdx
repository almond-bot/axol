...
| `--robot_config.rt true` | **Experimental.** Drive the wire through the `axol-rt` Rust realtime core: the core owns CAN at 240 Hz while the replay loop streams targets — same switch as [`teleop --rt`](/cli/teleop). Requires the `axol-rt` binary. |
