...
<Note>
  The hosted control panel resolves `--repo_id` against the configured dataset
  root only; passing a dataset *directory* as `--repo_id` is a direct-CLI
  shorthand.
</Note>

<Warning>
  Replay only datasets you created or obtained from a trusted source. The
  upstream dataset parser is not a sandbox for arbitrary HuggingFace
  repositories or filesystem directories supplied by someone else.
</Warning>

This command is configured via draccus. The robot subsystem is exposed as the nested `robot_config` config (CAN channels, per-joint gains) — nest into it with dots or pass a whole-config file. See [Command configuration](/cli/configuration) for the syntax and [Replay Dataset Config Parameters](/operations/replay-dataset-config) for every field with its default.
