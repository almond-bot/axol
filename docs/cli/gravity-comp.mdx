...
| `--telemetry_hz FLOAT` | Joint telemetry poll rate in Hz (default: 500). Ignored with `--rt`. |
| `--rt` | **Experimental.** Drive the wire through the `axol-rt` Rust realtime core: Python keeps computing the gravity model each cycle and streams the per-joint commands to the core, which owns CAN in both directions and sends telemetry back every tick. Same switch as [`teleop --rt`](/cli/teleop). Requires the `axol-rt` binary. |
