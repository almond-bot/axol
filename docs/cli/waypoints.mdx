...
| `--rt` | **Experimental.** Drive the wire through the `axol-rt` Rust realtime core: the core owns CAN at 240 Hz while teaching (gravity comp) and playback stream through it — same switch as [`teleop --rt`](/cli/teleop). Ignored with `--sim`. Requires the `axol-rt` binary. |
