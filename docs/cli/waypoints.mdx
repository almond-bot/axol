...
| `[` / `]` | Cycle the left / right gripper: limp → open → closed |
| `p` | Play the path |
...
Each gripper is worked on its own, so one arm can carry something while the other is still being guided. Open or close a gripper before recording and the waypoint stores that as the opening to command there; on playback the arms hold still while it moves, so a grasp closes on the object at the waypoint rather than somewhere along the way.

Grip force is the gripper's `torque_limit`, shared with every other operation and settable from the control panel. Force only exists where a command asks for more travel than the object allows, which is why closing records **fully closed** rather than wherever the fingers stopped — reproducing the stall position would bring them to rest against the object under no load at all. Set the torque for the object, not the opening.

A gripper you leave alone keeps whatever the last waypoint asked for, and playback does not pause to work it. So a pick-and-place is four waypoints and two gripper moves: open above the object, close on it, carry it, open to let go.

<Tip>
  Teaching starts with both grippers limp, so you can set the fingers by hand. The first `[` or `]` powers that gripper and opens it.
</Tip>
