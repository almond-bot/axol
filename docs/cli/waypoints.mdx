...
This command is configured via draccus — see [Command configuration](/cli/configuration). Arm gains and stiffness live on the nested `axol` config and IK weights on `kinematics` — the same fields as teleop's [Robot](/operations/teleop-config#robot) and [Kinematics](/operations/teleop-config#kinematics) parameters, with the same `--axol.` / `--kinematics.` prefixes. The command's own flags are listed below.
