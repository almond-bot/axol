...
class JointObservation:
    ...
    def note_command(self, ts: float) -> None:
        """Record that an external controller addressed this joint."""
        self.commanded_ts = ts

    def note_feedback(self, ts: float) -> None:
        """Timing is aggregated in Rust; state decoders only mark freshness."""
...
class BusObserver:
    ...
    def __init__(self, channel: str, joints: list[Joint]) -> None:
        ...
        self._timing_since: float | None = None
    ...
    async def start(self) -> None:
        ...
        self._bus.enable_observer_mode()
    ...
    def timing_snapshot(
        ...
        ``since`` resets the Rust reducer at a lifecycle boundary. The
        ``window_s`` and ``target_hz`` arguments remain for API compatibility;
        aggregation uses a fixed two-second window against the production
        240 Hz target. Returns ``None`` when command traffic is quiet. Every
        measurement uses the passive proxy's Rust receive clock.
        """
        del window_s, target_hz  # The Rust service owns the 2 s / 240 Hz contract.
        if self._bus is None:
            return None
        if since is not None and since != self._timing_since:
            self._bus.reset_timing()
            self._timing_since = since
            return None
        return self._bus.timing
