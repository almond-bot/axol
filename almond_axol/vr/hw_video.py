...
aiortc encodes video in software (libx264), which cannot keep up with
high-resolution camera relays on the Jetson's ARM cores. Jetsons ship a
hardware encoder (NVENC) exposed through GStreamer's ``nvv4l2h264enc``;
this module routes aiortc's H.264 encoding through it.
...
- :class:`JetsonH264Encoder` feeds the SDK's native **BGRA** frames to a
  ``gst-launch-1.0`` subprocess. ``nvvidconv`` does the only colorspace
  conversion (BGRx -> NV12) on the hardware VIC and ``nvv4l2h264enc``
  encodes; there is no CPU ``videoconvert``. The returned Annex-B byte
  stream is packetized with aiortc's own RTP packetizer. If the pipeline
  dies (e.g. a Jetson model without NVENC), it falls back to aiortc's
  software ``H264Encoder`` for the rest of the session.
...
# per frame, then idles until the next one). Flushing on idle avoids
...
# SDK frames are BGRA (4 bytes/pixel); the pipeline treats them as BGRx
# (alpha ignored) so nvvidconv converts straight to NV12 on the VIC.
_BYTES_PER_PIXEL = 4
...
def _gst_argv(width: int, height: int, fps: int, bitrate: int) -> list[str]:
    ...
    The input is the SDK's native BGRA, parsed as ``BGRx`` (the alpha byte is
    ignored). ``nvvidconv`` converts BGRx -> NV12 on the VIC — the only
    colorspace conversion in the path, on hardware — so there is no CPU
    ``videoconvert``. ``nvv4l2h264enc`` is the hardware encoder.
    ...
    """
    pipeline = [
        f"fdsrc fd=0 blocksize={width * height * _BYTES_PER_PIXEL}",
        (
            "rawvideoparse use-sink-caps=false format=bgrx "
...
class _GstH264Pipeline:
    """One ``gst-launch-1.0`` NVENC pipeline: BGRA frames in, H.264 AUs out.
    ...
    def write_frame(self, bgra: bytes | memoryview) -> None:
        """Feed one packed BGRA frame; raises if the pipeline has died."""
        assert self._proc.stdin is not None
        self._proc.stdin.write(bgra)
...
class JetsonH264Encoder:
    ...
    def _hw_encode(self, frame: av.VideoFrame) -> tuple[list[bytes], int]:
        ...
        # The track's frames are already BGRA, so this is a plain unpadded
        ...
        pipeline.write_frame(frame.to_ndarray(format="bgra"))
    ...
    def pack(self, packet: Any) -> tuple[list[bytes], int]:
        """Pre-encoded passthrough (unused here; kept for interface parity)."""
