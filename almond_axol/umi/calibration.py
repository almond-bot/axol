...
file at ``~/.almond/umi/tcp_transform.json`` takes precedence when present
...
``R_world_tcp = R_ctrl @ R(quat)``. ``apply_umi_teleop_profile`` resolves the
entry for the active tracker into ``VRTeleopConfig`` so both ``teleop --umi``
and ``collect-data --umi`` pick it up automatically.

The pre-keying legacy file format (``{"left": {"pos": ..., "quat": ...}}``,
written by the retired ``axol umi.calibrate`` robot-sweep command) is still
