...
def pin_realtime_clocks(*, interactive: bool = False) -> None:
    ...
    MAXN; uncaps the clock ceiling), pins NVENC/VIC/GPU (encode latency, ZED
    ...
    pinning is a no-op without the Tegra devfreq nodes. The power mode is
    selected first because it sets the ceiling the governor and engine pins
    reach; the interrupt step precedes the daemon step so the CAN receive path
