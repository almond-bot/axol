...
# Compute per-channel image stats on every Nth frame only. Stats feed dataset
# normalization and are robust to subsampling; doing every frame would put
# avoidable numpy/GIL load back into the recording path.
_STATS_EVERY = 4
...
class _CameraNvencEncoder:
    ...
    thread pads RGB->RGBA, writes the bytes to the gst subprocess, and computes
    sampled stats — keeping per-frame GIL work off the control loop.
    """
    ...
    def __init__(self, video_path: Path, fps: int, queue_maxsize: int) -> None:
        ...
        # Stats: reuse LeRobot's tracker so results match the software path.
