...
`{ "type": "joints", "value": { "q": { "<urdf joint name>": rad, ... }, "l_grip": 0..1, "r_grip": 0..1, "engaged": bool, "pair": { "aligned": bool, "width": m, "tilt": deg, "grasp": str } | null } }` streams the commanded arm joint state at ~20 Hz (measured positions when the robot reports them); `useAxolJoints` collects it, the **Ghost** overlay poses a translucent copy of the URDF from it, and `pair.aligned` (the IK worker's check that the grippers already form the box-mode pair: level, the tool's contact face toward each other across a box-sized gap) drives the **Box** button's *(aligned)* cue; `pair.width` is the grip width between the contact faces and `pair.tilt` (the pair's inward tilt trim, set by the sticks in box mode) with `pair.grasp` (`flush` / `straight`) are the button's readout while the mode is on. The URDF and its meshes are served by the same VR server under `https://<host>:8000/urdf/` (`axol.urdf`, `meshes/*.stl`) so the overlay needs no second origin.
