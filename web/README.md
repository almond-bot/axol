...
| Either thumbstick click | Toggle the **grasp**: *straight* (the default — fingers straight forward, 0°, width between the mounts; the parcel gripper's blade opens to its full 180° stroke here, and only here) or *flush* (the tool's contact face along the box side — the parcel gripper's blade opened 140°, 40° inward, width between the faces). The pair blends over; the **Box** button names the grasp |
