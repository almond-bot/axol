...
| Either thumbstick ↑ ↓ | How far out the **elbows** are held: push forward = further apart, pull back = tucked in (0–90° from straight down; kept for the next engage, shown as **Elbows out** in the session settings) |
| Either thumbstick click | Toggle the **grasp**: *straight* (the default — fingers straight forward, 0°, width between the mounts) or *flush* (the tool's contact face along the box side — the parcel gripper's blade held at 140° instead of its stop, yawed 40° inward, width between the faces). The pair blends over; the **Box** button names the grasp |

Both sticks do the same, clicks aren't modifiers (a single click-and-release toggles the grasp; one that becomes the both-sticks gesture doesn't), and the sticks never move the pair — only the leading hand does; a stick's dominant axis alone counts, so width and tilt never change together. The thumbsticks belong to the arms only *while a grip is leading*: press the leader's grip again to **freeze** the pair (the arms hold the box where it is) and the sticks drive Jelly with the normal mapping — so carrying is grab, freeze, drive, lead again to adjust. The hand-over waits until both sticks are released, so a stick still held can't become base motion; squeeze the trigger before taking the lead back, since the grippers follow it. Switching box mode engages at once: on, the pair is led by the right hand straight away (**Box lead hand** picks the left); off (same gesture, or the **Box** button), both arms engage in plain teleop — in ramp re-engage each blends back out to its controller by the mapping from before the box, which box mode leaves untouched. From the angled grasp while leading, the pair is levelled back to the straight grasp first (over `box_align_duration`) and the mode switches after, so that ramp starts from straight wrists. Going home (**X**) leaves box mode as well, without engaging.
...
**Live settings** ride the same socket in both directions. Any client sends `{ "type": "set", "key": "<key>", "value": <value> }` — a boolean key also takes `"value": "toggle"`, flipped server-side, which is what the both-sticks gesture sends — (handled by the callback from `VRServer.set_on_setting()`, i.e. `almond_axol.teleop.live.LiveSettings.apply`); the server validates it and answers every request — including no-ops and from every client's point of view — with `{ "type": "settings", "value": { "schema": [...], "values": { ... } } }`, also sent on connect (`set_announce("settings", ...)`). `schema` entries are `{ key, label, type: "boolean"|"select"|"number", help, options, min, max, step, unit }`; the headset's Settings panel and the control panel's Session settings card render from it. Keys: `box_mode`, `box_grasp` (`flush` / `straight`), `box_tool` (`parcel` / `urdf`), `box_tool_open_deg`, `box_face_left` / `box_face_right` (`auto` / `+x` / `-x`), `box_squeeze_force` (N, hardware only — box mode's clamp force per arm, shared over the gripper's contact points), `box_squeeze_torque` (Nm, hardware only — an opt-in per-joint shoulder backstop under it, off by default), `reengage`, `hold_to_engage`, `gripper_torque` (hardware only), `position_multiplier`, `teleop_max_vel` (rev/s).

`{ "type": "joints", "value": { "q": { "<urdf joint name>": rad, ... }, "l_grip": 0..1, "r_grip": 0..1, "engaged": bool, "pair": { "aligned": bool, "width": m, "grasp": str } | null } }` streams the commanded arm joint state at ~20 Hz (measured positions when the robot reports them); `useAxolJoints` collects it, the **Ghost** overlay poses a translucent copy of the URDF from it, and `pair.aligned` (the IK worker's check that the grippers already form the box-mode pair: level, the tool's contact face toward each other across a box-sized gap) drives the **Box** button's *(aligned)* cue; `pair.width` is the grip width between the contact faces and `pair.grasp` (`flush` / `straight`) is the button's readout while the mode is on. The URDF and its meshes are served by the same VR server under `https://<host>:8000/urdf/` (`axol.urdf`, `meshes/*.stl`) so the overlay needs no second origin.
